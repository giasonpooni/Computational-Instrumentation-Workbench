...
| `scripts/check_telemetry.py` | `src/ciw/pipelines/descriptors/telemetry.json` | `Provenance-Preserving-Data-Acquisition`, `Streaming-Telemetry-Feature-Extraction`, `Geometric-State-Inference-Engine`, `State-Estimation-Evaluation-Testbed`, `Constraint-Based-State-Reconciliation` |
| `scripts/check_calibrated_observable.py` | `src/ciw/pipelines/descriptors/calibrated-observable.json` | `fsrt`, `tbrt`, `mcur`, `oit`, `gsie`, `cbsr`, `fdir`, `set` |
