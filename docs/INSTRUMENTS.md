...
| Batch evaluation | `ciw-plsr-batch-v1`: a declared collection of samples evaluated through the same adapter, journalled durably, resumable and bounded by `--limit`, with one bundle per result and a `ciw-plsr-batch-index-v1` accounting for every requested sample; `python -m ciw plsr batch run examples/plsr/batch-parameter-sweep.json --output-dir results/sweep` |
| Batch comparison | `ciw-plsr-comparison-v1`: two saved batches compared from their retained bundles with no re-evaluation, reporting status transitions, diagnostic differences and changes to models, certificates, margins and runtime versions, with incompatible inputs stated explicitly; `python -m ciw plsr batch compare results/baseline results/candidate` |
| Status and exit codes | Raw runtime code and all three verdict booleans retained; exit `0` means command success, `2` input/configuration/execution failure, `3` replay mismatch, `4` a corpus case did not reproduce or a recording was refused, `5` the corpus is stale against the installed runtime, `6` a batch sample errored, `7` a batch did not finish |
...
One invocation evaluates one explicit sample, or a declared collection of them
as a bounded batch. A batch's `completed` count is accounting, not certification:
it includes `MARGIN_LOW`, and operationally acceptable samples are counted
separately. No parallelism, real-time loop performance or physical-system
validation is claimed; per-sample engine, adapter and evidence-writing times are
measured and reported in the batch index. A comparison establishes what changed
between two retained computations, not that either is physically valid. See the [guide's validation and limits](PLSR.md#validation-and-limits)
