...
| **Snapshot** | The `session.snapshot` event and `session.get` response `{session_id, run: RUN_METADATA, selection, results: [RESULT_SUMMARY], view_settings (M1)}`; in the extension also the immutable `(revision, watermark vector)` one render tick draws from. |
...
flowchart LR
    ...
    subgraph Clients
        ...
        AN[Headless analysis<br/>ciw analyze, in-process, no socket]
    ...
    AN -. session code in-process .-> REC
    HA --> ENG
    HA --> RB
...
| Terminal client (`send`, `watch`, `analyze`, panels) | Zero or more | Service restart (v1: `send` and `watch` exit with code 2 and are re-run; panels reconnect and request a snapshot from M1) |
...
The service is a separate process from both clients so that a client crash cannot delay sequencing and a stalled client is bounded (v1: by the 2 s delivery timeout in 5.3; from M1: cannot delay it, CIW-SYNC-021) and the same service serves both clients symmetrically. `ciw analyze` runs the session code headless and in-process. In-process instruments share the service's failure domain and are restricted to bounded calls; the subprocess binding is the default for anything else.
...
| Service ↔ in-process instrument | Python calls (8.1) | Budgeted by `max_call_ms` (8.11) |
...
|---|---|---|---|---|
| **Inline** | In-process binding (v1) | Synchronous calls with a declared budget `max_call_ms` (default 50 ms); an overrun beyond 10× the budget marks the instrument `Failed` for further calls while the running thread is abandoned, never killed; the service MUST cap abandoned threads (default 2) and refuse further in-process calls with `resource_exhausted` beyond that cap, naming the subprocess binding in the message | Recorded runs, small deterministic transforms, adapters | Anything that can block, allocate unboundedly, or crash |
...
| Service stops (SIGINT/SIGTERM on POSIX; native Windows: controller Stop) | Viewport shows STALE, disables shared interaction, offers Reconnect; `ciw send` exits 2; an operation in flight completes and its result is saved before exit (CIW-SESS-012); a request unanswered at exit is seen by its client as a disconnect | `ciw serve --resume` (or `--workspace`) restores the workspace saved at shutdown; `ciw health` confirms the session before clients reconnect |
...
`CIW-PERF-001` (v1) The v1 limits MUST be stated as limits: one uniformly sampled recording per session; only the built-in oscillator record: channels exactly `q` (m), `v` (m/s), `energy` (J), `coordinate_frame` `oscillator-state`, and the fixed `render` block (`axis_labels` `["q (m)", "energy (J)", "v (m/s)"]`, `trajectory`, `surface`, `sample_indices`, `transform`); `ciw serve --recording` and `ciw analyze --recording` accept a file written by `ciw demo` or one with that schema and refuse any other; text JSON only, 1 MiB maximum incoming message; 1,024 results per session; loopback only at the host boundary (container: `0.0.0.0` inside the namespace, published on host loopback), no authentication; no streaming, cancellation, binary arrays, spectrograms, uncertainty, or device acquisition; local JSON persistence only. New analyses produce no event in v1; clients refresh `result.list`.
...
|---|---|---|---|
| Manifest and adapter | Manifest, channel descriptors, capability-driven representations, subprocess binding in batch mode and SDK shim, existing-instrument adapter, operations as instrument code, one channel namespace, determinism harness, proto-check, session attachment of run bundles | INST-005, 006, 007 (batch lifecycle), 008 (batch families), 009, 013, 014, 016 (batch), 018 (subprocess), 019, 020 (attach, run, detach), DATA-003 (descriptor), 016 (bulk plane), SYNC-022, VIEW-001, EXT-001, 005 (a), 006 (a), 008, PERF-007 (instrument Ready), SESS-014 | M1 |
| Unit grammar and reductions | UCUM grammar and dimension vector, display-unit preferences, min/max reductions | DATA-008 (grammar), 009, 020, PERF-006 (decimation) | M1 |
| Result events and envelope v1.1 | `result.created` broadcast; protocol minor; uncertainty declarations, payload types, `reduced_of`, `checks[]`, `refused` status, stale marker; non-blocking broadcast queues | INST-003 (minor mechanism), DATA-013 (v1.1 fields), 015, 021 (session envelope `refused`), SYNC-016 (marker), 021 | M1 |
| Terminal panels | Textual panels, tiers, command grammar, keybindings, layouts, scripts, `assert`, render tick; `view.update` and `view.changed` | VIEW-003 (styling), 004..008, 009 (pane), 010, 011, 013 (pane), 016 (preview), SYNC-009 († fields), 010, 012, 013, 014, 017 (benchmark), OPS-002 (except `cancel`), 003 (form), 005, 006, 007 (except `x`), 008, 009, 010 (full), 011, PERF-002 (benchmark), 003 | M1 |
| Journal and session store | Sequence-numbered journal, session directory, replay, `verify` for operations, csv/arrow/npz exports | SESS-005, 006, 008, 009 (operations), 010 (csv, arrow, npz), SYNC-008, OPS-004, VIEW-022, EXT-003, DATA-019 (journal), PERF-008 (journal replay) | M2 |
| Persistence adapter | Resolver mapping the identities to an external infrastructure | EXT-004 | M2 |
| Uncertainty and revisions | Uncertainty forms and rendering, superseding results, `t_avail`, `as_of`, live parameters, `xy` plots, `verify` for instruments | DATA-003 (mandatory form), 010, 017, 018, INST-015, VIEW-003 (uncertainty), 012, 018, EXT-002, 005 (b), 006 (b), SESS-009 (instruments) | M3 |
| Binary frames and viewport resources | Bulk buffers with descriptor on the client link, coordinate-frame tree and gaps, resources by id, shared camera, rasterizer and graphics tier, gltf/png exports | DATA-011 (M4 part), 016 (client link), SYNC-009 (camera), 018, VIEW-006 (graphics), 016 (rasterizer), 017, 020, EXT-005 (c), 006 (c), SESS-010 (gltf, png), EXT-003 (gltf, png), PERF-011 (viewport query), PERF-002 (viewport), SYNC-017 (viewport) | M4 |
| Streaming | Credits, cancellation, heartbeats, remote and observed bindings, time bases, follow-live, recording, `host` channels | INST-007 (cancel, stream), 008 (stream types), 010..012, 016 (stream), 018 (remote, observed), 020 (credit, cancel), DATA-005..007, SYNC-007 (policy), 009 (follow), 011, 020, SESS-007, OPS-002 (`cancel`), 007 (`x`), PERF-004, 005, 010 | M5 |
| Spectral expansion | Welch, spectrogram, frequency cursor, link groups | SYNC-009 (freq_cursor), 015 (spectrogram), 019, VIEW-014, 015, EXT-008 (Welch), PERF-006 (Welch) | M5 |
| Deployment | Observed service class profile (R11), packaging, measured workloads, fault injection | PERF-009 | M6 |
...
`CIW-INST-005` (extension, M1) Every instrument MUST ship `instrument.json` with `manifest_version`, `id` (reverse-DNS; the instrument short id is the last label of `id` (`attitude-ekf` for `org.ciw.ref.attitude-ekf`) and MUST be unique among attached instruments; `attach` MUST be refused with `instrument_conflict` otherwise), `version` (semver), `title`, `binding ∈ {inprocess, subprocess, remote, observed}`, `entrypoint` or `endpoint`, `determinism` with optional `tolerance {abs, rel}`, `isolation`, `max_call_ms` (required for `binding: inprocess`, absent otherwise; the built-in oscillator's manifest declares 100 ms, the one-core budget of CIW-PERF-006 for a 1,000,000-sample PSD; 50 ms is the default for adapters that declare none), `heartbeat_ms`, `restart {policy, max}`, `parameters[]` (name, type, unit, default, range, `scope ∈ {execution, live}`, optional `source ∈ {selection.cursor, selection.interval}`), `inputs[]` (name, accepted kinds, dimension, shape, sampling, required), `outputs[]` (name, `mode ∈ {stream, batch}`, channel descriptors, `representations[]`, `overflow`), and `checks[]`. The service MUST refuse an instrument whose manifest fails schema validation and MUST record the manifest hash in provenance. JSON is the manifest format because it is the serialization the protocol and workspace already use and it embeds verbatim in `hello`.
...
| `contract` (bad seq, credit exceeded, descriptor mismatch, `call_overrun`) | yes (service-generated); `call_overrun` non-fatal below 10× the budget (5.5) | Execution failed; offending record logged |
...
|---|---|---|---|---|
| `inprocess` (v1) | Python calls with the same record shapes | NumPy arrays by reference | Declared budget `max_call_ms`, measured after each call; an overrun is journaled as a `contract` error `call_overrun`; the call is not pre-empted | None |
...
M2 moves the v1 flat files into `recordings/` and `results/`; a v1 output directory MUST still open.
...
The `frame` verb sets the selection's coordinate frame, never a data frame. A verb tagged M5 MUST be absent from `help --json` and MUST be rejected with `unsupported` before M5; before M5 an execution cannot be cancelled and a terminated subprocess closes with `status: failed` per CIW-INST-007.
...
| CIW-DATA-012 | unit: `sample_indices` increasing and in range; transform validity; integration: viewport cards equal `sample.get` | M0 | satisfied (oscillator schema only) |
...
| CIW-INST-007 | integration: `ciw-proto-check` drives every state; timeouts enforced; transitions journaled | M1 (batch), M5 (stream, cancel) | planned |
...
| CIW-INST-018 | integration: same suite over inprocess, subprocess, `ssh://localhost`; shim wraps in-process instrument; an in-process instrument that overruns 10× its budget is marked Failed and the abandoned-thread cap is enforced without delaying the sequencer | M1 (subprocess), M5 (remote) | planned |
...
| CIW-SYNC-021 | integration: a client that stops reading does not delay another client's accepted update beyond one tick; overflow and 2 s stall disconnect only that client | M1 | planned (v1 awaits delivery under the lock) |
| CIW-SYNC-022 | unit: duplicate `channel_id` refused with `channel_conflict`; bare ids resolve for run and result channels | M1 | planned |
...
| CIW-VIEW-006 | unit: probe ladder with mocked responses, including a mocked 300 ms round trip that still confirms tier 3; override; runtime fallback; 256 KiB cap; two-tick acceptance rule | M1, M4 | planned |
...
| CIW-SESS-002 | unit: invalid workspace rejected before any write; reopen runs no operation; new session id; identities intact; from M1 run check conditioned on `run_binding` | M0, M1 | satisfied (v1) / planned (`run_binding`) |
...
| CIW-SESS-014 | integration: an attached bundle is listed by `result.list` and returned by `result.get` with unchanged identities and no re-evaluation; reopening the workspace restores it; attached bundle with foreign `evidence_id` restored on reopen through its `bundle_digest`; `workspace_version: 2` refused by a v1 reader | M1 | planned |
| CIW-OPS-001 | integration: each subcommand; `send` exit codes; `inspect` executes nothing; `health` bounded at 3 s and read-only; `--bind` accepts only `127.0.0.1` and `0.0.0.0` | M0 | satisfied |
...
| CIW-EXT-003 | integration: csv, arrow, and npz exporters registered as plugins under `ciw.exports` without service change, each writing the CIW-SESS-010 sidecar (M2); gltf and png exporters as plugins (M4) | M2, M4 (gltf, png) | planned |
...
| CIW-EXT-007 | inspection: every contract in Sections 6-12 other than CIW-INST-004 and the `inprocess` binding row depends on no language runtime, toolkit, or renderer (byte formats permitted); extensions declare majors | M0 | satisfied |
...
| CIW-PERF-002 | benchmark: broadcast to output line (event lines, panels) and viewport frame | M1 (terminal), M4 (viewport) | planned |
...
| CIW-PERF-012 | unit: bind choices (`tests/test_deployment.py`); integration: `scripts/check_container.py` fresh Compose project on a temporary port and volume | M0 | satisfied |
...
| # | Decision | Rationale |
|---|---|---|
| D1 | Topology: an authoritative session service with the terminal and viewport as independent clients; the terminal works alone; closing the viewport never stops the service. | The v1 baseline and the Jupyter positioning; a client crash never delays sequencing; a stalled client is bounded to 2 s in v1 (5.3) and removed from the sequencing path at M1 (CIW-SYNC-021). |
...
| D8 | Intervals: half-open `[start, end)`; float seconds on the v1 fields (as built); from M5 per-base `int64` ns fields are authoritative, float fields are converted once by CIW-DATA-005 and both never appear in one update; the encoding of the ns fields is owned by `docs/PROTOCOL.md`. | Matches code and tests; nanosecond bases matter only with several clocks. |
