...
**Roadmap.** The prototype (v0.1) is milestone M0: the session service, both clients, the workspace, the two deployment paths with saved shutdown, resume, and a health probe, and the headless adapter for the Parameterized Lyapunov Stability Runtime (PLSR), Section 13.8. That adapter delivers the adapter part of M1 (an external engine called under a source pin with its status semantics preserved, runtime identity in provenance, separate identities, inspect versus replay) but not the manifest, the subprocess binding, session attachment, or capability-driven views. M1 completes the existing-instrument adapter: a manifest, the subprocess binding in batch mode, session attachment of run bundles, and the scalar/time-series reference instrument (a) behind an adapter with its headless calculation and tolerances preserved. M2 adds a persistence adapter that reuses an external evidence-and-result infrastructure's identities without merging them. M3 adds the state-estimation reference instrument with uncertainty and revisions; M4 the spatial reference instrument with binary frames on the client link and viewport resources; M5 spectrograms, streaming with separate acquisition, publish, and render rates, cancellation, and stale-result handling; M6 a deployment for a stated use with measured workloads. Readiness stages standalone-ready, workbench-ready, and deployment-ready are defined in Section 18.
...
| **Snapshot** | The `session.snapshot` event and `session.get` response `{session_id, run: RUN_METADATA, selection, results: [RESULT_SUMMARY], view_settings (M1), protocol_minor (M1)}`; in the extension also the immutable `(revision, W, t_tick, seen)` one render tick draws from (CIW-SYNC-010): watermark vector, tick clock, and first-seen tick per result. |
...
| **Channel** | A named, unit-bearing sequence of samples on one time base: `channels{name: {unit, values}}` in v1; a descriptor in the extension. Referenced by its bare `channel_id`, unique within a session (CIW-SYNC-022). |
...
| **Frame** (data) | One batch of samples with a descriptor; the unit of transfer in the streaming extension. Distinct from a *coordinate frame*, always written in full except as the `coordframe` verb (12.2). |
...
| **Run bundle** | A self-contained immutable JSON file written by the headless instrument path (8.13): inputs, output record, runtime identity, identities, digests. |
...
|---|---|---|---|
| attach a source | `ciw serve --recording`, `--workspace`, or `--resume` (v1: a recording in the oscillator schema only, CIW-PERF-001); `ciw plsr evaluate` (headless path, 8.13) | Session Service, Instrument | Instrument API (8.1), manifest (8.2), headless path (8.13) |
...
| inspect the result | `sample.get`, `result.get`, readouts, viewport cards | Terminal Client, Viewport | Representation layer (Section 10) |
...
flowchart LR
    ...
    subgraph Service["Session Service (ciw serve)"]
        ...
        AN[In-process host<br/>ciw analyze embeds the service, no socket]
    end
    subgraph Clients
...
| **Reducer** (extension) | Produces `derived` reductions at view resolution: decimation, resampling with a declared method, spectrogram column reduction, spatial level of detail; caches by key. Spectra and spectrograms are operations, not reductions; the reducer only fits their columns to a pane's width (CIW-SYNC-015). | Produce anything without provenance. |
| **Terminal Client** | Control surface: headless analysis, `send`, `watch`, `inspect`; panels, command line, keybindings, layouts, rasters (extension). | Compute a record as a client (headless analysis embeds the service, 5.2); hold data beyond what it displays. |
...
| **Headless adapter** (v1) | Drives an external instrument from the terminal client without the service; calls the upstream engine under a verified source pin; writes self-contained immutable run bundles carrying the identity model (8.13). | Attach to a session or publish to clients (extension, CIW-SESS-014); compute a verdict itself. |
...
| Terminal client (`send`, `watch`, `analyze`, panels) | Zero or more | Service restart (v1: `send` exits 2 when its request goes unanswered; `watch` exits 0 when the service closes the connection normally, including saved shutdown on SIGINT or SIGTERM, and 2 on an abnormal close such as forced termination; either is re-run to reattach; panels reconnect and request a snapshot from M1) |
...
| Deployment controller (`workbench.ps1`) or Compose | Zero or one per deployment | Service exit and restart (native: `Status` re-reads `service.json`; container: Compose restarts the service under `on-failure:3`); it holds no session state; ownership and stop behaviour are in 5.8 |

The service is a separate process from both clients so that a client crash cannot delay sequencing and a stalled client is bounded (v1: bounded only by the stall behaviour in 5.3, up to about 22 s; from M1: cannot delay it, CIW-SYNC-021) and the same service serves both clients symmetrically. `ciw analyze` embeds the Session Service in the terminal process without a socket: its record and result are produced by the service code, not by a client. In-process instruments share the service's failure domain and are restricted to bounded calls; the subprocess binding is the default for anything else.
...
Only the main thread mutates the selection; `selection.update` runs under an `asyncio.Lock` so broadcast order equals revision order across clients; results are registered by the operation's worker thread under the session lock after the result file write succeeds; operations and disk I/O run in worker threads so socket polling never blocks. In v1 the sequencer awaits delivery of `selection.changed` to every client under the lock; a send not completed within 2 s is abandoned and the sequencer then awaits the close handshake on that connection, still under the lock. When the client's receive window is full that close blocks in the transport's drain, which `close_timeout` does not bound; the connection is torn down only when the WebSocket keepalive next runs (`ping_interval` 20 s) and its send on the closing connection expires the 2 s close deadline. One stalled client therefore delays the next accepted update by up to about 22 s (measured 16.5 s and 19.6 s on loopback), and indefinitely if a keepalive ping was itself blocked behind the paused transport; CIW-PERF-002 does not hold for the other clients meanwhile. CIW-SYNC-021 removes this at M1. In the extension, ingest threads validate descriptors, place buffers, and post `frame_ready` to the main loop; reducer workers pull from a coalescing queue keyed by `(channel, client, resolution, window)`; the recorder has priority over chunk sealing.

### 5.4 IPC summary

| Link | v1 | Extension |
|---|---|---|
| Client ↔ service | Loopback WebSocket `ws://127.0.0.1:8765`, text JSON, no authentication, browser origins rejected, 1 MiB maximum incoming message; container bind `0.0.0.0` inside its namespace, published on the host's loopback only | Binary frames with descriptor on the same connection; Unix socket and TCP with token or TLS; SSH forwarding |
| Controller ↔ service | Native: `session.get` and `workspace.save` over the client link (`ciw send`), process ownership in `service.json`, stop by terminating the owned process after a verified save; container: Docker healthcheck through `ciw health`, SIGTERM with a 30 s grace period | Unchanged; no shutdown request is added to the client protocol (CIW-SESS-012) |
| Service ↔ in-process instrument | Python calls (8.1) | Budgeted by `max_call_ms` (CIW-INST-018) |
...
|---|---|---|---|---|
| **Inline** | In-process binding (v1 binding; budget and cap from M1) | Synchronous calls; from M1 budgeted by `max_call_ms` (default 50 ms), with the overrun, `Failed`, and abandoned-thread rules of CIW-INST-018 (an abandoned thread still competes for the GIL, which is why the cap is 2) | Recorded runs, small deterministic transforms, adapters | Anything that can block, allocate unboundedly, or crash |
...
| Service stops (SIGINT/SIGTERM on POSIX; native Windows: controller Stop) | Viewport shows STALE, disables shared interaction, offers Reconnect; `ciw send` exits 2; `ciw watch` ends with exit 0 (normal close); an operation in flight completes and its result is saved before exit (CIW-SESS-012); a request unanswered at exit is seen by its client as a disconnect | `ciw serve --resume` (or `--workspace`) restores the workspace saved at shutdown; `ciw health` confirms the session before clients reconnect |
| Service killed after the grace period, or power loss | Changes since the last completed save are lost | Save explicitly (`workspace.save`) before forced termination; `--resume` reopens the last save |
| Invalid request (`invalid_request`, `unknown_command`, `invalid_payload`, `not_found`; the v1 code set of CIW-INST-002) | `type: "error"` with `{code, message}`; session unaffected | Client corrects and retries |
...
| Instrument exits or misses 3 heartbeats (extension) | Instrument `Failed`; channels frozen and marked stale; open executions closed with `status: failed` and `truncated_at {frame_seq, reason: instrument_failed \| heartbeat_loss}` | Manual restart by default; bounded `auto` (8.8) |
...
Two deployment paths share the same service and protocol. The native controller (`scripts/workbench.ps1`: Setup, Start, Status, Stop) runs the service as a hidden process on `ws://127.0.0.1:8765` with a `.ciw/` data directory holding recordings, results, `workspace.json`, logs, and `service.json` ownership metadata; Stop saves and verifies the workspace and stops only the owned process; Start restores a saved workspace and errors on corrupt data rather than starting a new investigation. The container backend (Compose service `backend`) runs `ciw serve --bind 0.0.0.0 --resume --output-dir /data`; the host publishes only `127.0.0.1:${CIW_PORT:-8765}`; a named volume `workspace` holds `/data`; read-only root filesystem, tmpfs `/tmp`, all capabilities dropped, no-new-privileges, non-root uid 10001, `stop_grace_period` 30 s, `restart: on-failure:3`, Docker healthcheck via `ciw health`. See [`deploy/README.md`](../deploy/README.md) and [`deploy/CONTAINER.md`](../deploy/CONTAINER.md).

`CIW-SESS-012` (v1) On SIGINT or SIGTERM the service MUST stop accepting connections, drain clients, finish in-flight operations, save `workspace.json` to the output directory, then exit. There is no shutdown request in the client protocol. Forced termination, termination after a container grace period, and power loss are not covered by autosave; `workspace.save` is the explicit checkpoint. `serve --resume` MUST reopen `<output-dir>/workspace.json` when present, MUST start a new investigation only when no workspace exists, and MUST fail on corrupt or incompatible saved data. A restarted service assigns a new runtime `session_id` (v1 as built; clients MUST NOT treat it as persistent); run, evidence, execution, and result identities MUST NOT change across restart. The signal path is POSIX. On Windows, process termination is immediate and not preceded by a save; console SIGINT (Ctrl+C) saves; the native controller's Stop (explicit `workspace.save`, verified against the live session, then termination of the owned process only) is the supported saved-shutdown path, and a Windows service stopped by any other means counts as forced termination.
...
`CIW-INST-002` (v1) The service MUST implement the commands below. Requests with an unsupported `protocol_version`, extra envelope fields, a missing or over-long `request_id`, unknown payload fields, non-finite numbers, or booleans where integers are expected MUST be rejected with a structured error and MUST leave the session unchanged. The v1 error codes are `invalid_request` (malformed envelope or non-JSON/binary frame), `unsupported_version`, `unknown_command`, `invalid_payload`, `revision_conflict`, `not_found`, `capacity_exceeded`, and `storage_error`; a client MUST treat an unlisted code as an error of unknown class.
...
`CIW-PERF-001` (v1) The v1 limits MUST be stated as limits: one uniformly sampled recording per session; only the built-in oscillator record: channels exactly `q` (m), `v` (m/s), `energy` (J), `coordinate_frame` `oscillator-state`, and the fixed `render` block (`axis_labels` `["q (m)", "energy (J)", "v (m/s)"]`, `trajectory`, `surface`, `sample_indices`, `transform`); `ciw serve --recording` and `ciw analyze --recording` accept a file written by `ciw demo` or one with that schema and refuse any other; text JSON only, 1 MiB maximum incoming message; 1,024 results per session; loopback only at the host boundary (container: `0.0.0.0` inside the namespace, published on host loopback), no authentication; no streaming, cancellation, binary arrays, spectrograms, uncertainty, or device acquisition; local JSON persistence only. New analyses produce no event in v1 (CIW-INST-023 adds one at M1); v1 clients refresh `result.list`.

### 6.2 Extension policy

`CIW-INST-003` (v1; minor mechanism M1) The client session protocol's `protocol_version` MUST be an integer major (1); a service MUST reject a request whose major it does not implement with `unsupported_version`. A service MUST reject unknown request fields (CIW-INST-002); in v1 as built every unknown field is rejected. From M1 the service MUST report `protocol_minor` in `session.snapshot`; an addition within a major MUST be an optional field or a new message type listed per minor in `docs/PROTOCOL.md` (the declaration form is owned there); a service MUST accept and ignore a declared optional request field it does not implement and MUST continue to reject undeclared fields; a client MUST ignore unknown optional fields and unknown broadcast types in responses and broadcasts; any change to the meaning of an existing field or message type MUST increment the major. The instrument link (Section 8) carries its own version (CIW-INST-019); the two version spaces are independent.

`CIW-INST-023` (extension, M1) After a result is registered (result file written, ledger updated under the session lock) the service MUST broadcast `result.created` with `request_id: null` and payload RESULT_SUMMARY (including `source_kind`) to every connected client through the per-client queue of CIW-SYNC-021, in the order results were registered and selection updates applied for that client; the requesting client MUST receive its response before its copy of the broadcast; an attached bundle (CIW-SESS-014) produces the same broadcast. A client MUST treat a missed or out-of-order broadcast as a reason to refresh `result.list` or request a snapshot, never as data loss; the result file exists before the broadcast is queued.

Minors within these majors are declared per extension in `docs/PROTOCOL.md` (CIW-INST-003); a row that required a new major would say so here.

| Extension | Adds | Requirements | Milestone | Majors (protocol / envelope / manifest) |
|---|---|---|---|---|
| Manifest and adapter | Manifest, channel descriptors, capability-driven representations, subprocess binding in batch mode and SDK shim, existing-instrument adapter, operations as instrument code, one channel namespace, determinism harness, proto-check, session attachment of run bundles | INST-005, 006, 007 (batch lifecycle), 008 (batch families), 009, 013, 014, 016 (batch), 018 (subprocess; in-process budget and cap), 019, 020 (attach, run, detach), INST-022 (attach, detach, run, result.attach), DATA-003 (descriptor), 016 (bulk plane), SYNC-022, VIEW-001, EXT-001, 005 (a), 006 (a), 008, PERF-007 (instrument Ready), SESS-014 | M1 | 1 / 1 / 1 |
| Unit grammar and reductions | UCUM grammar and dimension vector, display-unit preferences, min/max reductions | DATA-008 (grammar), 009, 020, PERF-006 (decimation) | M1 | 1 / 1 / 1 |
| Result events and envelope v1.1 | `result.created` broadcast; protocol minor; uncertainty declarations, payload types, `reduced_of`, `checks[]`, `refused` status, stale marker; non-blocking broadcast queues | INST-023 (result.created), INST-003 (minor mechanism), DATA-013 (v1.1 fields), 015, 021 (session envelope `refused`), SYNC-016 (marker), 021 | M1 | 1 / 1 / 1 |
| Terminal panels | Textual panels, tiers, command grammar, keybindings, layouts, scripts, `assert`, render tick; `view.update` and `view.changed` | VIEW-003 (styling), 004..008, 009 (pane), 010, 011, 013 (pane), 016 (preview), SYNC-009 († fields), 010, 012, 013, 014, 017 (benchmark), OPS-002 (except `cancel`), 003 (form), 005, 006, 007 (except `x`), 008, 009, 010 (full), 011, PERF-002 (benchmark), 003 | M1 | 1 / 1 / 1 |
| Journal and session store | Sequence-numbered journal, session directory, replay, `verify` for operations, csv/arrow/npz exports | SESS-005, 006, 008, 009 (operations), 010 (json, csv, arrow, npz), SYNC-008, OPS-004, VIEW-022, EXT-003, DATA-019 (journal), PERF-008 (journal replay) | M2 | 1 / 1 / 1 |
| Persistence adapter | Resolver mapping the identities to an external infrastructure | EXT-004 | M2 | 1 / 1 / 1 |
| Uncertainty and revisions | Uncertainty forms and rendering, superseding results, `t_avail`, `as_of`, live parameters, `xy` plots, `verify` for instruments | DATA-003 (mandatory form), 010, 017, 018, INST-015, VIEW-003 (uncertainty), 012, 018, EXT-002, 005 (b), 006 (b), SESS-009 (instruments) | M3 | 1 / 1 / 1 |
| Binary frames and viewport resources | Bulk buffers with descriptor on the client link, coordinate-frame tree and gaps, resources by id, shared camera, rasterizer and graphics tier, gltf/png exports | DATA-011 (M4 part), 012 (resources), 016 (client link), INST-022 (subscribe, credit), SYNC-009 (camera), 018, VIEW-006 (graphics), 016 (rasterizer), 017, 020, EXT-005 (c), 006 (c), SESS-010 (gltf, png), EXT-003 (gltf, png), PERF-011 (viewport query), PERF-002 (viewport), SYNC-017 (viewport) | M4 | 1 / 1 / 1 |
| Streaming | Credits, cancellation, heartbeats, remote and observed bindings, time bases, follow-live, recording, `host` channels | INST-007 (cancel, stream), 008 (stream types), 010..012, 022 (cancel), 016 (stream), 018 (remote, observed), 020 (credit, cancel), DATA-005..007, SYNC-007 (policy), 009 (follow), 011, 020, SESS-007, OPS-002 (`cancel`), 007 (`x`), PERF-004, 005, 010 | M5 | 1 / 1 / 1 |
| Spectral expansion | Welch, spectrogram, frequency cursor, link groups | SYNC-009 (freq_cursor), 015 (spectrogram), 019, VIEW-014, 015, EXT-008 (Welch), PERF-006 (Welch) | M5 | 1 / 1 / 1 |
| Deployment | Observed service class profile (R11), packaging, measured workloads, fault injection | PERF-009 | M6 | 1 / 1 / 1 |
...
`CIW-DATA-003` (extension, M1; uncertainty form mandatory M3) From M1 a channel descriptor MUST carry `channel_id`, `kind`, `dtype`, `shape` (inner shape per sample), `unit`, `time_base`, `coordinate_frame` or `null`, `missing`, and `uncertainty` (a form, `none`, or absent); it MUST carry `components[]` when `shape` is non-scalar and MAY carry `cursor_policy` (`nearest` when omitted; `at_or_before` per CIW-SYNC-007). From M3 an `estimate` channel MUST carry a form or `none` with a `reason`. `representations[]` is declared per output (CIW-INST-005, CIW-INST-006) and per result envelope (CIW-DATA-013), never per channel, and applies to every channel of that output. `kind` MUST be `observation`, `estimate`, `derived`, or `reference`; any producer MAY emit any kind. A `derived` channel MUST carry `derived_from[]` naming its inputs' kinds, so a quantity derived from an estimate (innovation, residual, normalized innovation squared) is never presented as an observation. `reference` marks ground-truth data used for comparison.

A source instrument declares the kind of what it emits: recorded or synthetic evidence that stands in for a measurement is `observation` (the v0.1 oscillator's `q`, `v`, and `energy` stand in for measurements, its provenance says so, and they carry kind `observation` once descriptors exist at M1); the output of an estimator, reconstruction, or integrator run inside the session is `estimate`.
...
`CIW-DATA-005` (extension, M5) A time base MUST declare `id`, `clock ∈ {monotonic, wall, sim}`, `epoch` (ISO 8601 with offset for `wall`; session origin for `monotonic`; model t = 0 for `sim`), and `resolution_ns`; streamed timestamps MUST be `int64` nanoseconds relative to the epoch. Bases are declared by a source in its data `envelope` (`time_bases[]`) or by a run in `metadata.time_bases[]`; the service registers them by `id` and refuses a redeclaration with different fields (`time_base_conflict`). The reserved base `run` is the v1 recording base: `clock` per `provenance.time_reference` (`sim` for synthetic runs), `epoch` = run start, `resolution_ns` = 1, `time_s` mapped by `t_ns = round_half_even(time_s × 1e9)`. In a manifest, `input:<name>` MUST resolve at attach to the base of the channel bound to that input. The v1 selection fields `cursor_s` and `interval_s` remain float64 seconds on the run's base (`CIW-DATA-004`). From M5 the authoritative cursor and interval are the per-base `int64` ns fields of CIW-SYNC-020; `cursor_s` and `interval_s` are projections computed by the service. This is a protocol minor, not a major: in a session with one time base, `cursor_s` and `interval_s` keep their v1 meaning and are accepted and reported unchanged; `cursors` and `intervals` are optional additions that are authoritative whenever present and are the only accepted form for a base that fails the precision bound. A v1 client that never sends the ns fields observes no change. A `selection.update` MAY carry either the float fields (converted once by the rule below against the run's base) or the ns fields, never both in one update (`invalid_payload`). For a channel in an `int64` base the service MUST convert each interval bound once, `t_ns = round_half_even(t_s × 1e9)`, and evaluate membership in integers as `start_ns ≤ t_ns < end_ns`; the result envelope MUST record the executed `interval_ns`, whether converted or received, alongside `interval_s`, and `ciw verify` MUST re-execute from `interval_ns`. A base's `epoch` MUST be chosen so that float64 seconds resolve its `resolution_ns`: max |t_s| over the run measured from the epoch × 2^-52 ≤ resolution_ns × 1e-9 (for 1 ns, |t_s| ≤ 4.5e6 s); a base that cannot meet this MUST be addressed through the ns fields only, and the service MUST reject a float update for it with `precision_insufficient`. Channels in different bases MUST NOT be combined unless a `mapping {offset_ns, drift_ppb, valid_from, source, version}` has been declared with evidence; the default is no mapping, the cursor is then per base, and views show `unmapped`. A device-clocked source MUST send `clock_sync {tb_ns, host_ns}` at least every 10 s; the service MUST fit offset and drift over the last 60 s, store raw timestamps unmodified with the mapping alongside, and version each refit so alignment can improve without rewriting data.

`CIW-DATA-006` (extension, M5) A channel MUST declare `regular {rate, phase, jitter_tol}` (`jitter_tol` default 1 % of the nominal sample period), `irregular`, or `event` sampling; regular frames MAY carry `(t0, dt, n)`. Timestamps MUST be non-decreasing within a channel (`time_order` on violation); equal timestamps are preserved in arrival order; a regular sample off its grid by more than `jitter_tol` MUST be rejected with `sampling_grid` unless a gap precedes it. Spectral operations MUST require regular sampling.

`CIW-DATA-007` (extension, M5) The service MUST NOT resample implicitly. Resampling MUST be an explicit derivation whose provenance names the method (`hold`, `linear`, `nearest`, `pchip`) and `gap_policy` (default `nan`). Uncertainty handling MUST be defined per method: `hold` and `nearest` copy σ; `linear` combines the neighbours' variances. A spectral operation on an `irregular` or `event` channel MUST be refused with `sampling_irregular`; the operator MAY first create an explicit `resample` derivation (method and target rate recorded in provenance) and run the spectrum on it. Default methods when a resample derivation is requested: `linear` for observations (including residuals between observations), `none` for estimates (refused unless a method is named).
...
`CIW-DATA-010` (extension, M3) Uncertainty, when present, MUST be `stddev`, `variance`, `covariance` (default layout full row-major `[d, d]` per sample; `packed_lower` and `block_diagonal` MAY be declared), `interval {lower, upper, level}`, `quantiles`, or `ensemble {members}`. Absent uncertainty MUST be absent, never zero. Uncertainty MUST travel as companion columns of the same channel with the same time column. Covariance MUST be symmetric positive semi-definite; the service SHOULD check symmetry and MAY check semi-definiteness by sampled Cholesky, reporting failures as channel-scoped warnings. A unit-quaternion state MUST declare its covariance in the tangent space (`[3, 3]`, `rad2`). A companion column is addressed as `<channel_id>.<column>` with the column name fixed by the form: `.stddev`, `.variance`, `.covariance`, `.lower` and `.upper` (`interval`), `.quantiles` (one column with the levels in its descriptor), `.ensemble` (members on the inner axis); the missing-data mask of CIW-DATA-011 is `<channel_id>.mask`. A companion column is not a channel: it is implied by the channel's declared `uncertainty` or `missing`, MUST NOT be listed in a frame's `channel_ids[]`, appears in `buffers[]` as `{channel_id, column}` (CIW-DATA-016), and its `unit` and `shape` MUST equal the declared form's. A manifest MUST NOT declare a `channel_id` equal to another channel's companion name; the boundary check of CIW-INST-019 MUST accept exactly these derived columns and no other undeclared column.
...
`CIW-DATA-013` (v1; fields added M1) A result MUST be `{result_id, evidence_id, operation_id, execution_id, verification_id, verification_status, run_id, selection_revision, channel, interval_s, created_at, recording_file, data}`. From M1 (envelope v1.1) the run-bound fields `run_id, selection_revision, channel, interval_s, recording_file` are grouped as `run_binding`, required for results of `analysis.*` and other operations on the session run, and `null` for an attached run bundle, which instead carries `source {kind: "bundle", bundle_file, bundle_digest}`; v1 results keep the flat fields. The five identity fields (`evidence_id`, `operation_id`, `execution_id`, `result_id`, `verification_id`) MUST be distinct fields, never derived from one another. Two executions of one operation on one evidence MUST share `operation_id` and `evidence_id` and differ in `execution_id` and `result_id`; from M1 an operation with parameters MUST fold a canonical parameter hash into `operation_id`. `verification_id` MUST be `null` and `verification_status` `not_verified` until a distinct verification execution references the result; verification MUST never be inferred from a displayed scene. From M1 the envelope MUST also carry `envelope_version`, `kind`, `channels[]` (descriptors with units, shapes, time base, frame, missing convention, uncertainty), `payload {type}`, `representations[]`, `status ∈ {partial, complete, refused, cancelled, failed}` (`refused` per CIW-DATA-021), and optionally `reduced_of`, `supersedes {result_id, interval}`, `checks[]` (instrument self-checks with outcomes; they do not set `verification_id`), and `truncated_at {frame_seq, reason ∈ {cancel, instrument_failed, heartbeat_loss, transport, retention}}`, the last valid `frame_seq` when an execution ended before `end_of_stream`. RESULT_SUMMARY is the identity and binding fields of a result without `data`: `result_id, operation_id, execution_id, channel, interval_s, created_at, verification_status, selection_revision` in v1 (the exact set is fixed by `docs/PROTOCOL.md`); from M1 also `source_kind` (CIW-SESS-014) and `status`.
...
Example v1 result as written to `<output-dir>/result-<id>.json`, with envelope v1.1 fields marked `+`:
...
```json
{
  ...
  // addressed as "q.stats:execution-9d0e5b4a3c2f1e8d7b6a5c4f3e2d1b0a"
  "+payload": {"type": "table"},
...
`CIW-DATA-016` (extension, M1 instrument bulk plane; M4 client link) A bulk frame MUST carry a descriptor sufficient to interpret its buffers without the envelope: `frame_seq`, `execution_id`, `stream` (the manifest output, as `<instrument short id>/<output name>`), `result_id` or `channel_ids[]`, and per buffer `dtype`, `shape`, `byte_order` (`little`), `order` (`C`), `byte_offset`, `byte_length`, `unit`, `coordinate_frame` (required for a spatial channel buffer; `null` or absent otherwise, matching the channel descriptor of CIW-DATA-003), plus `time {column}` or `time {t0, dt, n}` with its `time_base`. The default transfer and at-rest format is NumPy-compatible raw contiguous buffers with this descriptor; Arrow IPC (pyarrow) is the format for table payloads and export and MAY be negotiated per link. Buffers MUST NOT be embedded in text records. Each buffer MUST name `channel_id` (`null` for the time column, which carries no `column`) and, for a channel buffer, `column ∈ {value, stddev, variance, covariance, lower, upper, quantiles, ensemble, mask}` (further column names only for forms `CIW-DATA-010` declares); companion columns are not channels and MUST NOT appear in `channel_ids[]`. On the client link bulk frames share the connection with control messages, so a control broadcast waits behind the bulk in flight. A frame or resource message on the client link MUST be at most 1 MiB (larger resources are sent in chunks); the bulk ahead of any control message is bounded by the subscription's outstanding credit (default 8 frames / 8 MiB, sized for loopback); a client MUST grant credit that its link drains within the 2 s of CIW-SYNC-021 and SHOULD grant no more than its measured link rate drains in 1 s over an SSH tunnel.

Instrument-link `data/frame` record (CIW-INST-008) announcing a frame from the state-estimation reference instrument; the service forwards the same `payload` to subscribed clients as a `frame` broadcast of the client session protocol (a minor addition at M4, `bulk` replaced by the binary WebSocket message that follows), framing per `docs/PROTOCOL.md`:
...
```json
{
  ...
  "payload": {
    ...
    "buffers": [
      ...
      {"channel_id": "att.bias", "column": "value",      "dtype": "f64", "shape": [256, 3],    "byte_order": "little", "order": "C", "byte_offset": 28672, "byte_length": 6144,  "unit": "rad/s", "coordinate_frame": null},
      {"channel_id": "att.bias", "column": "stddev",     "dtype": "f64", "shape": [256, 3],    "byte_order": "little", "order": "C", "byte_offset": 34816, "byte_length": 6144,  "unit": "rad/s", "coordinate_frame": null}
    ],
    "bulk": {"binding": "shm", "segment": "ciw-7f3a-000091", "byte_length": 40960}
  }
}
```
...
`CIW-DATA-019` (v1; journal M2) `workspace_version`, `protocol_version`, and from M1 `envelope_version` and `manifest_version` MUST be checked on read; a reader MUST refuse a higher major version and, from M1 when a second workspace major exists, MUST name the converter. `metadata.provenance` MUST name generator and version, dtype, time reference, and sampling convention, and for an external instrument the runtime identity of CIW-EXT-009. From M2 the journal MUST record for every execution the operation, parameters, seed, input identities, and manifest hash. From M2 the journal MUST record for every execution, and `ciw verify` for its own run, a `platform {os, arch, python, numpy, scipy, blas {name, version}, fft {name}, threads}` record.
...
`CIW-INST-005` (extension, M1) Every instrument MUST ship `instrument.json` with `manifest_version`, `id` (reverse-DNS; the instrument short id is the last label of `id` (`attitude-ekf` for `org.ciw.ref.attitude-ekf`) and MUST be unique among attached instruments; `attach` MUST be refused with `instrument_conflict` otherwise), `version` (semver), `title`, `binding ∈ {inprocess, subprocess, remote, observed}`, `entrypoint` or `endpoint`, `determinism` with optional `tolerance {abs, rel}`, `isolation`, `max_call_ms` (`binding: inprocess` only, optional, default 50 ms; absent for other bindings; the built-in oscillator's manifest declares 100 ms, twice the default, because its periodogram over the whole run executes synchronously on one core), `heartbeat_ms`, `restart {policy, max}`, `parameters[]` (name, type, unit, default, range, `scope ∈ {execution, live}`, optional `source ∈ {selection.cursor, selection.interval}`), `inputs[]` (name, accepted kinds, dimension, shape, sampling, required), `outputs[]` (name, `mode ∈ {stream, batch}`, channel descriptors, `representations[]`, `overflow`), and `checks[]`. The service MUST refuse an instrument whose manifest fails schema validation and MUST record the manifest hash in provenance. JSON is the manifest format because it is the serialization the protocol and workspace already use and it embeds verbatim in `hello`.
  ...
  "outputs": [
    {"name": "state", "mode": "stream", "representations": ["numerical", "temporal", "spatial3d"], "overflow": "block",
     "channels": [
       ...
       {"channel_id": "att.bias", "kind": "estimate", "dtype": "f64", "shape": [3], "components": ["x", "y", "z"], "unit": "rad/s", "coordinate_frame": null,
...
At M3 both outputs run in `batch` mode over the recorded input; `mode: stream` and `overflow` take effect at M5 when the instrument consumes live sources.
...
`CIW-INST-007` (extension, M1 batch lifecycle for the subprocess binding: Discovered, Attaching, Ready, Running, Draining, Detaching, Failed with `hello`, `ready`, `heartbeat`, `progress`, `finalized` and their timeouts; M5 Cancelling, `cancel_ack`, and stream mode) An instrument MUST follow the state machine above with these obligations and timeouts. Attaching: reply to `hello` with `manifest` within 5 s, verified against the on-disk copy. Ready: validate parameters and reply `ready` within 30 s of `attach`. Running: consume and emit as credits allow; `heartbeat` every `heartbeat_ms`; `progress` at least every 1 s in batch mode. Draining: emit remaining outputs, `end_of_stream` per output, then `finalized` with per-output content digests, within 30 s. On expiry the service MUST (M1) terminate the process group, move the instrument to Failed, and close the envelope with `status: failed` and `truncated_at {frame_seq: <last valid frame_seq>, reason: instrument_failed}`; from M5 it MUST first send `cancel` and apply CIW-INST-012, terminating only if `cancel_ack` is not received within `2 × heartbeat_ms`, in which case the same Failed closure applies. Cancelling: stop producing (target 200 ms) and send `cancel_ack` within `2 × heartbeat_ms` (default 2 s); frames already sent remain valid; the envelope status becomes `cancelled` with `truncated_at {frame_seq, reason: cancel}`. Failed: emit `error(fatal)` before exit; the service retains everything received and closes the envelope with `status: failed` and `truncated_at` set to the last valid `frame_seq`. Each `run` MUST carry a new `execution_id`; every transition MUST be journaled; concurrent executions require `concurrency > 1` in the manifest.
...
| `transport` | yes (service-generated) | Envelope closed with `status: failed` and `truncated_at {reason: transport}` |
...
`CIW-INST-015` (extension, M3) A parameter with `scope: execution` MUST NOT affect a running execution; it applies to the next `run` and yields a new `operation_id`. A `scope: live` parameter MAY change during an execution; every change MUST be journaled and recorded in the envelope's parameter history.
...
`CIW-INST-018` (extension, M1 subprocess; M5 remote, observed) Semantics MUST be identical across bindings; bindings differ only in transport. An instrument MUST run under `subprocess` and `remote` without code change; an in-process instrument MUST be wrappable as a subprocess by a generic SDK shim. For the `inprocess` binding, from M1 the service MUST measure each call against the instrument's `max_call_ms` (CIW-INST-005; default 50 ms); an overrun MUST be journaled as a `contract` error `call_overrun` (non-fatal below 10× the budget); an overrun beyond 10× the budget MUST mark the instrument `Failed` for further calls while the running thread is abandoned, never killed; the service MUST cap abandoned threads (default 2) and MUST refuse further in-process calls with `resource_exhausted` beyond that cap, naming the subprocess binding in the message. The caller stops awaiting the call at 10× the budget; a value an abandoned thread returns later MUST be discarded and journaled, never registered as a result. `resource_exhausted` is an M1 addition to the error codes of CIW-INST-002 under the policy of CIW-INST-003.

| Binding | Control plane | Bulk plane | Liveness | Isolation |
|---|---|---|---|---|
| `inprocess` (v1 binding; budget M1) | Python calls with the same record shapes | NumPy arrays by reference | v1: none (bounded calls by convention, CIW-INST-004). From M1 (CIW-INST-018): declared budget `max_call_ms` measured after each call; an overrun is journaled as a `contract` error `call_overrun`; the call is not pre-empted | None |
...
`CIW-INST-019` (extension, M1) `hello` on the instrument link MUST carry `instrument_protocol {major, minor}`, independent of the client session `protocol_version`; majors MUST match or the link is refused with `unsupported_version`; the minor in effect is the lower of the two; unknown fields on the instrument link MUST be ignored; manifest, envelope, and both protocol versions are independent. The service MUST enforce at the boundary: every frame column is either a channel declared in the current envelope or a companion column of one (CIW-DATA-010); every buffer's byte length equals `dtype size × product(shape)`; timestamps are non-decreasing; the envelope's `kind` matches every channel's; every unit parses; every spatial channel names a declared coordinate frame.

`CIW-INST-020` (extension, M1; M5 credit and cancel) `docs/PROTOCOL.md` MUST specify the concrete encoding for every requirement in this section and MUST ship a harness (`ciw-proto-check <manifest>`) that drives an instrument through attach, run (batch), and detach at M1, with credit exhaustion and cancel added to the harness at M5, and reports pass or fail per identifier.

`CIW-INST-022` (extension, M1 attach/detach/run/result.attach; M4 subscribe/credit; M5 cancel) The client session protocol MUST carry the operation loop as request types with these required fields and results, encoded per `docs/PROTOCOL.md` as minor additions under CIW-INST-003: M1 `instrument.attach {id | manifest_path, inputs{name: channel_id}} → {short_id, state}` (errors `instrument_conflict`, `channel_conflict`, `manifest_invalid`), `instrument.detach {short_id} → {state}`, `execution.run {short_id, parameters{}, interval_s?} → {execution_id}` (asynchronous per CIW-OPS-003; completion is the `result.created` broadcast of CIW-INST-023), `result.attach {bundle_file} → RESULT_SUMMARY` (CIW-SESS-014), and broadcasts `instrument.changed {short_id, state}` and `execution.changed {execution_id, status}`; M4 `stream.subscribe {stream | channel_ids[], resolution} → {subscription_id}`, `stream.unsubscribe {subscription_id}`, and `stream.credit {subscription_id, frames, bytes}` (CIW-INST-010 defaults); M5 `execution.cancel {execution_id} → {status}` (CIW-INST-012; `unsupported` before M5). Every such request MUST be sequenced and journaled (CIW-OPS-004) and MUST leave the selection unchanged.

### 8.13 Headless instrument path

`CIW-INST-021` (v1) An instrument MAY be driven from the terminal client without the session service. Such a run MUST be written as one self-contained immutable JSON bundle embedding the complete input declaration, the explicit sample or inputs, the instrument's own output record, and the runtime identity; it MUST carry separate `evidence_id`, `operation_id`, `execution_id`, `result_id`, and `verification_id` (`null`, `verification_status: "not_verified"`) and content digests whose relationships are validated on read; `inspect` MUST validate a bundle without recomputation; `replay` MUST re-evaluate the embedded inputs under new execution and result identities, record `replay_of` source bindings and an exact digest comparison, and MUST NOT modify the source bundle or promote verification. The adapter obligations toward the upstream engine are CIW-EXT-009; the catalogue rule for listing such an instrument as integrated is CIW-EXT-010.

`CIW-SESS-014` (extension, M1) A run bundle MAY be attached to a session. The service MUST register it as an immutable result of the workspace with its `evidence_id`, `operation_id`, `execution_id`, `result_id`, `verification_id`, and `verification_status` unchanged and its `bundle_digest` recorded as the source binding; MUST NOT re-evaluate it on attach or on reopen; MUST list it through `result.list` and return it through `result.get` as a result whose `payload` is a `reference` to the bundle (CIW-DATA-015) with the instrument's status record as the inline summary; and MUST offer views for it only per CIW-VIEW-001. `results[]` MAY therefore hold results whose `evidence_id` differs from `run.evidence_id`; RESULT_SUMMARY MUST carry `source_kind ∈ {run, bundle}`. A workspace holding an attached bundle MUST be saved with `workspace_version: 2` so that a v1 reader refuses it by CIW-SESS-004 naming the version rather than by a failed result check; a v2 reader MUST accept `workspace_version: 1`. Until then a bundle is inspected and replayed only through the headless path. Attach is the request `result.attach {bundle_file}` (protocol minor, M1; wire form in `docs/PROTOCOL.md`; CIW-INST-022). On attach the service MUST copy the bundle unchanged into the output directory under its own basename (`run-<result uuid>.json`, 13.8; M2: `bundles/run-<result uuid>.json`), MUST refuse with `storage_error` when a file of that name exists with different content, MUST record `bundle_file` relative to the output directory exactly as `recording_file`, and MUST verify `bundle_digest` on the copy before registering the result. Attaching a bundle whose `result_id` or `execution_id` is already present in the ledger MUST be refused with `invalid_payload` and leave the session unchanged. For a row with `source_kind: bundle`, RESULT_SUMMARY keeps the v1 flat fields with `channel`, `interval_s`, and `selection_revision` set to `null`; the fields keep their v1 meaning wherever non-null, so this is protocol minor 1.1 under CIW-INST-003, and a client MUST tolerate `null` in these fields only when `source_kind` is present.

## 9. Synchronization model

### 9.1 The shared selection

`CIW-SYNC-001` (v1) A session MUST have exactly one selection, held by the service; clients MUST NOT keep a divergent copy, and every change MUST pass through `selection.update` or, from M5, is the service's own follow mutation of CIW-SYNC-011, sequenced identically.

`CIW-SYNC-002` (v1) The selection MUST be `{run_id, channel, interval_s: [start, end), cursor_s, coordinate_frame, revision}`; the initial selection MUST be the run's default channel over the full run with the cursor at the first sample and `revision: 0`; in v1 the default channel is `q` (the oscillator displacement, fixed by `docs/PROTOCOL.md`); from M1 it is the first channel declared by the run's instrument manifest output.
...
`CIW-SYNC-022` (extension, M1) Channel references are bare `channel_id`s in one session namespace: the dot in `att.quat` is part of the id, never a namespace separator. Every `channel_id` MUST be unique across the session's run and the declared outputs of every attached instrument; the service MUST refuse `attach` with `channel_conflict` when an instrument declares an id already present. The selection's `channel`, every binding in `view_settings.channels`, and every channel argument in Section 12.2 MUST use this form. Channels produced by an execution belong to that execution's result (`channels[]`, CIW-DATA-013) and become addressable when the envelope is committed; `run_id` continues to name the session's recording, and in v1 the namespace is that run's channels. A channel produced by an execution is addressed as `<channel_id>:<execution_id>` (the qualified form; `:` never appears inside a `channel_id`). A bare `channel_id` that is declared only by the run resolves to the run channel. A bare `channel_id` that is produced by results MUST be resolved by the service when the `selection.update`, `view.update`, or command is accepted, to the channel of the most recently committed non-superseded result declaring it (journal order from M2; `created_at` at M1), and the selection, `view_settings.channels`, and every broadcast MUST carry the qualified form so that a later execution never changes what an existing revision denotes; `compare` pairs and residuals MUST be rendered with the qualified form; `assert` expressions combine the qualifier and the cursor suffix as `att.quat:execution-9d0e…@cursor`.

### 9.2 Revisions and conflict

`CIW-SYNC-003` (v1) Every `selection.update` MUST carry `expected_revision`; the service MUST apply it only if equal to the current revision, MUST increment `revision` by exactly one, and MUST reject a stale update with `revision_conflict` without mutating anything. An update that names no field besides `expected_revision` MUST be rejected with `invalid_payload`; an update that restates the current values MUST be accepted and MUST increment `revision` (v1 as built); revisions MUST be integers compared numerically and booleans rejected.
...
`CIW-SYNC-009` (extension; † M1, camera M4, follow and freq_cursor M5) `view_settings` (empty in v1) MUST hold exactly the fields below when populated; a change to any of them except `focus` MUST be a revisioned update like the selection and MUST never alter stored data. View settings and the selection share one revision counter. A change MUST be sent as `view.update {expected_revision, <fields>}`, applied by the selection sequencer under the same lock with the same conflict rule as CIW-SYNC-003, and broadcast as `view.changed` carrying the full `view_settings` (without `focus`) and the new `revision`; from M1 `session.snapshot` and the `session.get` response MUST include `view_settings` alongside `protocol_minor` (CIW-INST-003). `focus` is per-client presentation state: it is never broadcast, is persisted only as the saving client's value, and MUST NOT change in another client. One sequenced mutation MAY change selection fields and view-settings fields together under one revision (CIW-SYNC-011); `view.changed` MUST carry `{selection, view_settings, revision}`, and a mutation that changes both MUST be broadcast as a single `view.changed` (no separate `selection.changed`), so the discard rule of CIW-SYNC-004 applies unchanged.
...
`CIW-SYNC-010` (extension, M1) All panes rendered in one tick MUST render from one snapshot `(revision, W, t_tick, seen)`: `W` is the per-channel watermarks at the start of the tick; `t_tick` is the client's monotonic clock in ns captured once at the start of the tick; `seen` maps each `result_id` visible to the client to the `t_tick` of the first snapshot that included it, maintained by the snapshot builder, never by a renderer. A pane MUST NOT read live state while rendering. Every rendered or exported frame MUST be attributable to its `(revision, W, t_tick)`; the status line SHOULD show the revision and the focused channel's watermark lag.

`CIW-SYNC-011` (extension, M5) Data arrival MUST NOT mutate the selection or view settings directly. The service is the only follow source: while `follow ≠ off`, the selection sequencer MUST apply at most one follow mutation per `follow_period_ms` (default 50 ms), computed from the watermarks at that instant, moving `window` and `cursor` together: `edge` moves `end_s` to the newest watermark keeping the width and places the cursor there; `offset` keeps the cursor `lag_s` behind. The mutation is sequenced under the selection lock, increments `revision` once, is journaled with origin `system`, and is broadcast per CIW-SYNC-009 below. Clients MUST NOT send follow-derived updates. When the sequencer accepts an explicit `selection.update` carrying `cursor_s` (or `cursors`, CIW-SYNC-020) or an explicit `view.update` carrying `window` while `follow ≠ off`, it MUST set `follow: off` in the same sequenced mutation under the same revision.
...
`CIW-SYNC-015` (v1 rule; spectrogram M5) A spectral result MUST be computed by the service from the retained samples of exactly the interval captured in the result, never by a client and never from the cursor; the temporal pane MUST shade that interval, snapped inward to sample boundaries. A spectrogram's time axis MUST coincide with the temporal `window`; each column MUST be timestamped at the centre of its segment; `spectrum.stft.v1` MUST take `segment` and `hop` as explicit operation parameters (defaults 1024 and `segment / 4`) folded into `operation_id`; the result's columns are fixed by those parameters and the interval; a spectrogram pane MUST reduce result columns to its bucket width through the reducer (per-bucket maximum and minimum per frequency row, CIW-DATA-020) and MUST NOT request a recompute to fit its width; the temporal cursor MUST map to the nearest column centre, shown in the readout. `freq_cursor` MUST be shared by all spectral panes and read out at or before the bin; the frequency axis MUST derive from the actual sampling in the interval.
...
`CIW-SYNC-020` (extension, M5) With channels in more than one time base selected, the selection MUST hold one cursor per base, linked only through declared mappings. From M5 the selection MUST carry `cursors {<time_base_id>: {cursor_ns}}` and `intervals {<time_base_id>: [start_ns, end_ns)}` alongside the v1 fields; `cursors` and `intervals` are authoritative; `cursor_s` and `interval_s` are service-computed projections onto the run's base, accepted in an update only under the conversion rule of CIW-DATA-005 and never together with the ns fields. Cursor resolution converts `cursor_s` to `cursor_ns` with the bound conversion of CIW-DATA-005 before the nearest-sample rule of CIW-SYNC-007.

`CIW-SYNC-021` (extension, M1) The sequencer MUST NOT await network delivery. After applying an update it MUST append the broadcast to a per-client bounded outbound queue (default 64 messages) and return; a client whose queue overflows or that does not drain a message within 2 s MUST be disconnected off the sequencing path; per-client order is preserved by the queue. The disconnect MUST abort the transport from a task scheduled off the sequencing path; the sequencer MUST NOT await a close handshake or a drain under the lock. The 2 s drain budget applies per message, bulk frames included; a client whose credit exceeds what its link drains in 2 s is disconnected by this rule, not exempted.

## 10. Representation layer

### 10.1 Capability-driven views; clients own no calculation

`CIW-VIEW-001` (extension, M1) A view family MUST be offered for an output only if its `representations[]` includes it; no client or service MAY synthesize a spectral or spatial view an output does not declare. At M1 the demo instrument's manifest declares `numerical`, `temporal`, `spectral`, and `spatial3d`; in v1 the families are implied by the run's channels and `render` block.
...
`CIW-VIEW-006` (extension, M1 tiers 0–2; M4 graphics) The terminal client MUST classify the terminal at startup and on resize and select the highest confirmed tier, overridable by `--tier`: 0 cell; 1 braille 2×4 and half-block 1×2 with 256 colours; 2 true colour; 3 a graphics protocol in the order `kitty > iterm2 > sixel`, detected by terminal response: DA1 is sent first and its round trip measured; each graphics query then waits max(100 ms, 4 × DA1 round trip), capped at 1 s; a query that times out leaves the tier unconfirmed and the status line shows `tier 2 (unconfirmed 3)`; under tmux or screen tier 3 requires passthrough and defaults to 2 (Kitty via the APC graphics query, Sixel via DA1 attribute 4; iTerm2 has no graphics query and is the one environment-variable exception: `TERM_PROGRAM=iTerm.app` or `LC_TERMINAL=iTerm2` confirms it, and a failed first image write falls back one tier), never by `TERM` alone. A backend returning an error at runtime MUST fall back one tier. Tier-3 payloads MUST be at most 256 KiB per pane per frame; a pane whose tier-3 write does not complete within one tick period on two consecutive ticks (Kitty additionally on a missing `q=0` acknowledgement within one tick) drops to tier 2 for the rest of the session.
...
`CIW-VIEW-012` (extension, M3) A region superseded by a revision result MUST be marked on the time axis on every tick for which `t_tick − seen[superseding result_id] < 1 s`, both taken from the snapshot, and on at least the first such tick; a value resolved from a superseded result MUST be flagged `superseded` unless `as_of` selects it.
...
`CIW-VIEW-018` (extension, M3) An `xy` phase-plot representation of family `spatial2d` (one channel against another over the window, time as colour) MUST be available for any two channels whose `representations[]` both include `temporal` and whose time bases are compatible; the viewport's phase portrait is its first instance.
...
`CIW-SESS-002` (v1) Reopening MUST validate the entire workspace before any write and restore run, selection, and results without invoking any operation; a new `session_id` MUST be assigned while run, evidence, result, and execution identities stay intact. Saved results MUST be checked against the run (`run_id`, `evidence_id`, `recording_file`, channel and interval validity, `sample_count` equal to the retained samples in the interval, `not_verified` with `verification_id: null`, no duplicate ids) and, per operation, for the internal consistency of `data` without recomputation: for `statistics.v1` exactly the fields of CIW-INST-004, `unit` equal to the channel's, `minimum ≤ mean ≤ maximum` (1e-12 relative tolerance), and `rms ≥ 0`; for `spectrum.periodogram.v1` `method`, `window`, `detrend`, and `scaling` fixed to the v1 values, `sample_count ≥ 4`, `sample_rate_hz` equal to the run's, `unit` equal to `(<unit>)^2/Hz`, arrays of length `sample_count // 2 + 1`, `frequency_hz[i] = i · sample_rate_hz / sample_count`, non-negative `psd`, and `peak_frequency_hz` equal to the frequency of the maximum (`null` when the PSD is all zero); a result failing any check MUST be rejected. From M1 the run check applies to results with a non-null `run_binding`; a result with `source.kind: "bundle"` MUST instead be checked by re-reading the bundle file, verifying its `bundle_digest` and the digest relations of CIW-INST-021 without recomputation, and confirming the envelope identities equal the bundle's.
...
`CIW-SESS-004` (v1 workspace; result files M1) A workspace file MUST carry `workspace_version`; from M1 a result file MUST carry `envelope_version` (CIW-DATA-013). A reader MUST refuse a newer major and accept older ones it lists as supported. In v1 as built a result file carries no version field and is validated through the workspace that embeds it.
...
A reader MUST open a v1 flat output directory (`workspace.json`, `recording-<hash>.json`, `result-<id>.json` beside it, and from M1 `run-<result uuid>.json`) as well as this layout. M2 moves the v1 flat files into `recordings/` and `results/`. `workspace.json` carries `time_bases[]`, each element a declaration of CIW-DATA-005 (`{id, clock, epoch, resolution_ns, mapping?}`), and `coordinate_frames[]`, each a declaration of CIW-DATA-011.
...
`CIW-SESS-007` (extension, M5) Recording is not a mode: everything the service accepts MUST be recorded; ingested frames MUST be recorded losslessly through the credit path; sealed chunks MUST be content-hashed and the hash journaled. A `retain` policy (`all` default, `sources`, `window:<duration>`) governs space, never correctness. A sealed chunk MUST NOT be deleted while any execution input cursor or client subscription position lies at or before it; retention is bounded by the slowest reader. A reader more than `retain.max_lag` behind the newest sealed chunk (default: the `window` duration; unbounded under `all`) MUST have its input closed with `truncated_at {frame_seq, reason: retention}` and the event journaled; deletion under `sources` applies the same rule to derived chunks.
...
`CIW-SESS-009` (extension, M2 operations; M3 instruments) `ciw verify <session>` MUST: (1) leave nondeterministic instruments unstarted and use their recordings; (2) re-execute deterministic and seeded operations and instruments in topological order of provenance, in batch mode, from recorded inputs, parameters, and seeds; (3) compare each new result to the recorded one by digest (bitwise without tolerance) or element-wise within `tolerance`, NaN equal to NaN; (4) report any mismatch as `NONREPRODUCIBLE` with the first differing `(frame_seq, row)` and a drift report `{channel, execution_old, execution_new, max_abs_diff}`; (5) write its outputs as new results in a new session referencing the original by hash. Exit codes follow CIW-OPS-010: 0 reproducible, 2 command error, 3 nonreproducible, 4 instrument failure, 5 storage error. A mismatch on an instrument or operation with a declared `tolerance` is always `NONREPRODUCIBLE` (exit 3). A mismatch on a bitwise-declared instrument is `DRIFT`, reported with the drift record and exit 0, only when the recorded and current `platform` records differ; with identical platform records it is `NONREPRODUCIBLE`. `--strict` makes every mismatch exit 3.

### 11.6 Export

`CIW-SESS-010` (extension, M2; gltf and png M4) Exports MUST include, from M2, `json` (one file per result or per workspace, the result and workspace records in their stored form), `csv` (one file per time base, columns in declared or preferred units, `σ` columns, units in a header row), `arrow` (Arrow IPC per result), `npz`, and from M4 `gltf` (meshes, trajectories, point clouds) and `png`/`txt` (pane rasters per `CIW-VIEW-022`). Every export MUST write `<name>.export.json` with the command, journal sequence, selection revision, interval, unit preferences applied, and the identities of every exported result; an export in the declared unit MUST reproduce stored values exactly. v1 has no export command; its persisted `result-<id>.json`, `recording-<hash>.json`, and `workspace.json` files are readable without the service under CIW-SESS-001 and are not exports.
...
`CIW-OPS-001` (v1) The terminal client MUST provide `ciw demo` (write a deterministic synthetic recording), `ciw analyze stats|spectrum --recording --channel --start --end --output-dir` (headless analysis writing a recording copy, an immutable result, and a reopenable `workspace.json`), `ciw serve --recording|--workspace|--resume --output-dir --port --bind {127.0.0.1|0.0.0.0}` (CIW-SESS-012, CIW-PERF-012), `ciw send <type> --payload|--payload-file [--url]` (one request, JSON response on stdout, exit 0 on `response` and 2 on `error`), `ciw watch [--url]` (snapshot and events as JSON lines; exit 0 when the service closes the connection normally, 2 on an abnormal close), `ciw health [--url]` (CIW-SESS-013); `--url` defaults to `ws://127.0.0.1:8765` for `send`, `watch`, and `health`, `ciw inspect <path>` (print a saved file without executing anything), and `ciw plsr import|evaluate|inspect|replay` (8.13; optional `plsr` extra; exit 0 when the command succeeds even if the record holds a refusal or violation, 2 on input, configuration, or execution failure, 3 on replay digest mismatch). Every operator action MUST be expressible as one of these or, from M1, as a command below. The v1 terminal client prints JSON and event lines only; panels and in-terminal rasters are extensions (Sections 10.3, 10.7).

### 12.2 Command model

`CIW-OPS-002` (extension, M1) The grammar MUST be `verb [object] [--flag value]…` with `"` quoting, `#` comments, and time literals `<n>s|ms|us|ns`, ISO 8601 with `@`, `now`, `end`, `sel.start`, `sel.end`; verbs MUST map to the operation loop. A verb tagged M5 MUST be absent from `help --json` and MUST be rejected with `unsupported` before M5. Each verb of the attach and run rows maps to the request types of CIW-INST-022; the select rows map to `selection.update` and `view.update`.
...
Before M5 an execution cannot be cancelled and a terminated subprocess closes with `status: failed` per CIW-INST-007.

`CIW-OPS-003` (v1 envelope; M1 form) Every command MUST produce exactly one structured result, `{id, ok: true, value}` or `{id, ok: false, error: {code, message, detail}}`. In v1 `ciw send` and `ciw analyze` emit the protocol envelope as this result; other subcommands print a command-specific JSON object on success and a one-line `ciw: <message>` on stderr with exit 2 on failure; the uniform form is M1. The command line renders it; headless mode writes one JSON line; the gateway returns it as the response. Side effects MUST complete before the result unless the command is documented as asynchronous, in which case `value` MUST carry an `execution_id`.
...
|---|---|---|---|
| (a) `org.ciw.ref.timeseries` (M1): the built-in oscillator (`analytic-damped-oscillator.v1`) qualifies as the scalar/time-series reference instrument and remains required for the reusability test; at M1 it is placed behind a manifest and adapter with its calculation unchanged. PLSR (13.8) does not replace it | source, `deterministic` | scalar observations, regular sampling | manifest, adapter, numerical/temporal/spectral panels, preserved tolerances, reopen versus recompute |
...
`CIW-EXT-007` (v1) The architecture MUST remain valid if any of the three layers (engines, terminal client, viewport) is replaced. Every contract in Sections 6-12 MUST be implementable from any language: it MUST NOT depend on a language runtime, toolkit, or renderer, and named byte formats (NumPy-compatible contiguous layout, Arrow IPC) are permitted only as layouts a replacement can read. The in-process binding (CIW-INST-004 and the `inprocess` row of 8.11) is host-language specific by definition and is the only language-bound contract; a replacement host provides its own in-process binding behind the same manifest. Every extension MUST declare the protocol, envelope, and manifest majors it targets.
...
`CIW-EXT-008` (extension, M1) An operation (`statistics.v1`, `spectrum.periodogram.v1`, and the `spectrum.welch.v1` and `spectrum.stft.v1` operations of `CIW-VIEW-015`) MUST be supplied by instrument code: in v1 by the functions of `CIW-INST-004`; from M1 by an instrument registered per `CIW-EXT-001` whose manifest declares a `batch` output naming the `operation_id`. The service's Operation runner MUST only sequence the execution, pass the explicit interval and parameters, wrap `data` in the envelope, assign identities, and persist; it MUST NOT contain numerical code. Adding an operation MUST NOT change `src/ciw/session.py` beyond registration.

### 13.8 First external adapter: PLSR (v1, headless)

The Parameterized Lyapunov Stability Runtime (PLSR) evaluates quadratic Lyapunov certificates for declared linear and affine-parameter models; it belongs to the dynamical-system/observer candidate family (13.6) and is the first external adapter, decided in [ADR-0004](adr/0004-plsr-terminal-adapter.md) and documented in [`docs/PLSR.md`](PLSR.md) and [`docs/INSTRUMENTS.md`](INSTRUMENTS.md). Commands: `ciw plsr import | evaluate | inspect | replay`; optional `plsr` extra pinned to an upstream commit, Python 3.12+ (the base workbench stays 3.11); adapter `ciw-plsr-adapter-v1`; operation `plsr.verdict.v1`; input `plsr-sample-v1` (explicit `x`, `theta`, `theta_dot`, no defaults); saved run `ciw-plsr-run-v1` (`run-<result uuid>.json`, the UUID suffix of `result_id`, written atomically and never overwritten with different content) embedding the complete model declaration, the sample, the runtime companion record, and the runtime identity, with `evidence_id` a digest over the model `artifact_digest` and the sample, and with model `artifact_digest`, companion `record_digest`, and CIW `bundle_digest` validated against one another; `verification_status: "not_verified"`, `may_authorize: false`, physical validation `not_started`, `proof_status: NOT_CHECKED`; exit codes 0 (command succeeded; the record may hold a refusal or violation), 2 (input, configuration, or execution failure), 3 (replay digest mismatch); one explicit sample per invocation; no shared session, no viewport, no change to protocol v1; numerical views only. M1 items delivered by this adapter at M0: existing-instrument adapter with the upstream engine and its status semantics preserved (CIW-EXT-009); runtime pin in provenance; separate identities; inspect versus replay (CIW-INST-021); catalogue entry (CIW-EXT-010). M1 items remaining: manifest and schema (CIW-INST-005), session attachment of bundles (CIW-SESS-014), capability-driven views (CIW-VIEW-001), the subprocess binding (CIW-INST-018), and the reference instrument (a) adapter with its reusability report (CIW-EXT-005, CIW-EXT-006).
...
| Client transport | Loopback WebSocket `ws://127.0.0.1:8765`, text JSON (v1); binary frames on the same connection (M4); Unix socket and TCP with token or TLS (M5) | One transport both clients speak; forwardable over SSH; a browser client would cost nothing at the protocol level; control and bulk share the connection, head-of-line delay bounded by the client's credit (CIW-DATA-016). |
...
The tree below is reproduced unchanged from the development guide, whose annotations it carries ('This guide' names `docs/DEVELOPMENT.md`):
...
| Terminal client: CLI, service control (`serve`, `health`), headless instrument path, command grammar, scripts, `assert` | `src/ciw/cli.py` |
| Headless instrument adapter (PLSR): commands, run bundles, runtime pin | `src/ciw/` (`plsr.py`, `plsr_engine.py`) |
...
| Godot bridge check, container lifecycle and installed-package checks, native deployment controller (`workbench.ps1`), benchmark runners | `scripts/` |
| Unit, integration, and conformance tests (one module per area, named by identifier) | `tests/`, `tests/conformance/` |
| Reference instruments (a), (b), (c) with manifests and headless tests | `examples/` (alongside the existing `examples/plsr/` fixtures) |
| PLSR example model artifacts and samples (v1) | `examples/plsr/` |
| Wire specification, quickstart, instrument catalogue, PLSR operating guide, decisions | `docs/` |
| Container backend image and Compose service | `Dockerfile`, `compose.yaml` |
| Deployment guides (native controller, container backend) | `deploy/` |
...
`CIW-PERF-004` (extension, M5) Ingest MUST sustain 1,000,000 scalar float64 samples per second across local subprocess instruments with recording on, using ≤ 30 % of one core for ingest and recording, for 10 minutes without credit starvation. Load definition: four subprocess instruments at 250,000 samples/s each in frames of 4,096 samples (about 61 frames/s per instrument); smaller frames are outside the budget; the CPU share is the ingest and recorder threads' CPU time divided by wall time. Live latency from an instrument's send to the reduced frame at a local client MUST be ≤ 150 ms at p95 under that load.

`CIW-PERF-005` (extension, M5) Resident memory MUST stay within `memory_budget_bytes` (default 2 GiB) plus 256 MiB of interpreter overhead for one hour under `CIW-PERF-004`; eviction order is mapped chunks least recently subscribed, then reducer cache, then unreferenced resources; unsealed ring data MUST never be evicted. Eviction here means memory residency of mapped chunks, which remain re-readable from disk; deletion is governed only by the `retain` policy of CIW-SESS-007.

`CIW-PERF-006` (extension, M1 decimation; M5 Welch) A min/max decimation of 10,000,000 samples to 2,000 columns MUST complete in ≤ 40 ms and a Welch PSD of 1,000,000 samples in ≤ 100 ms, each on one core, as the median of at least 20 runs after 5 warm-up runs.
...
| integration | multi-process check: service plus clients, subprocess or remote instruments, headless scripts, the Godot bridge check, the container lifecycle check, and the installed-package check |
| golden | raster comparison against stored text or PNG at fixed size and tier |
| benchmark | two runs: (a) CI regression, on the CI machine class against a recorded baseline, failing on a regression above 20 %, never asserting the budget; (b) budget assertion, on a named profile runner (self-hosted, or a documented machine whose report is attached to the milestone record), which is what a PERF row must pass to close a milestone |
...
| CIW-DATA-003 | unit: descriptor schema; `cursor_policy` defaults to `nearest`; derived without `derived_from` rejected (M1); estimate without form rejected (M3); integration: estimate-derived labelled in both frontends | M1 (descriptor), M3 (mandatory form) | planned |
...
| CIW-DATA-005 | unit: refusal without mapping; 60 s fit; raw timestamps unchanged; bound conversion and integer membership at a boundary sample; update with both forms rejected; float update on a base outside the precision bound rejected; integration: two bases show `unmapped` | M5 | planned |
| CIW-DATA-006 | unit: `time_order` and `sampling_grid`; equal timestamps preserved | M5 | planned |
| CIW-DATA-007 | unit: no implicit resample path; spectrum on irregular channel refused with `sampling_irregular`; σ per method; gap policy; integration: residual provenance | M5 | planned |
...
| CIW-DATA-010 | unit: each form; absent never zero; symmetry; sampled Cholesky; tangent-space quaternion covariance; companion naming per form; undeclared column rejected | M3 | planned |
| CIW-DATA-011 | unit: v1 frame names match (M0); coordinate-frame declaration; overlay refusal; missing convention; gaps persisted, including `retention` (M4) | M0 (v1 rule), M4 | satisfied (oscillator schema only; v1 rule) / planned (M4) |
...
| CIW-DATA-016 | unit: descriptor completeness; byte length equals dtype × shape; integration: subprocess and viewport round trip | M1 (subprocess), M4 (viewport) | planned |
| CIW-DATA-017 | unit: superseding leaves original intact; `t_valid`/`t_avail`; `as_of` selects availability | M3 | planned |
| CIW-DATA-018 | unit: `propagation` declared on operation results; `σ-DROPPED` rendered for `dropped`; golden: a temporal pane over an estimate with declared σ shows the `sigma_k` band, never the flag | M3 | planned |
...
| CIW-DATA-021 | unit: `tests/test_plsr.py` domain refusals (`OUTSIDE_PARAMETER_BOX`, `OUTSIDE_LEVEL_SET`) saved with exit 0; `NUMERICAL_INCONCLUSIVE` (`numerical_refusal`) distinct from `NOT_CERTIFIED` (`certificate_violation`); `NUMERICAL_OVERFLOW` retained as a sampleless refusal; malformed samples are errors that write nothing | M0 (headless bundles), M1 (session envelope `refused`) | satisfied (PLSR) / planned (session results) |
...
| CIW-INST-007 | integration: `ciw-proto-check` drives every state; timeouts enforced; transitions journaled; M1: `ciw-proto-check` withholds `finalized` past 30 s and asserts Failed, `status: failed`, `truncated_at.reason = instrument_failed`; M5: the same case first sends `cancel` | M1 (batch), M5 (stream, cancel) | planned |
...
| CIW-INST-018 | integration: same suite over inprocess, subprocess, `ssh://localhost`; shim wraps in-process instrument; an in-process instrument that overruns 10× its budget is marked Failed and the abandoned-thread cap and `resource_exhausted` refusal of CIW-INST-018 are enforced without delaying the sequencer | M1 (subprocess), M5 (remote) | planned |
...
| CIW-INST-023 | integration: a second client receives `result.created` after `result-<id>.json` exists and after the requester's response; payload equals the `result.list` row; attached bundle broadcasts with `source_kind: bundle`; a client that missed it recovers via `result.list` | M1 | planned |
...
| CIW-SYNC-003 | unit: stale revision rejected without mutation; increments once; booleans rejected; field-less update rejected | M0 | satisfied |
...
| CIW-SYNC-009 | unit: field set and defaults; `view.update` revisioned under the selection lock; `view.changed` broadcast; snapshot carries view settings; `focus` not broadcast | M1 (†), M4, M5 | planned (reserved `{}`) |
| CIW-SYNC-010 | unit + golden: panes render from injected snapshot only; frame attributable | M1 | planned |
| CIW-SYNC-011 | unit: arrival leaves revision unchanged with follow off; ≤ 1 follow mutation per `follow_period_ms` with follow on, origin `system`; explicit cursor or window update sets `follow: off` in one revision broadcast as one `view.changed`; integration: two following clients send no updates and stay at one revision | M5 | planned |
...
| CIW-SYNC-017 | unit: rule holds by construction of CIW-SYNC-004 (M0); benchmark: with CIW-PERF-002 (M1 terminal, M4 viewport) | M0 (rule), M1 (terminal), M4 (viewport) | satisfied (rule) / planned (benchmark) |
...
| CIW-SYNC-021 | integration: a client that stops reading does not delay another client's accepted update beyond one tick; overflow and 2 s stall disconnect only that client | M1 | planned (v1 awaits delivery and then the close handshake under the lock) |
| CIW-SYNC-022 | unit: duplicate `channel_id` refused with `channel_conflict`; bare ids resolve for run and result channels; two executions of one instrument: a bare id resolves to the latest at acceptance and the broadcast carries the qualified form, each qualified id resolves to its own execution, and the selection at an earlier revision is unchanged by a later commit | M1 | planned |
...
| CIW-VIEW-006 | unit: probe ladder with mocked responses, including a mocked 300 ms round trip that still confirms tier 3; override; iTerm2 environment-variable confirmation and first-write fallback; runtime fallback; 256 KiB cap; two-tick acceptance rule | M1, M4 | planned |
...
| CIW-SESS-002 | unit: invalid workspace rejected before any write; reopen runs no operation; per-operation consistency of saved `data` (`tests/test_replay.py`); new session id; identities intact; from M1 run check conditioned on `run_binding` | M0, M1 | satisfied (v1) / planned (`run_binding`) |
...
| CIW-SESS-007 | integration: recorder backpressure; chunk hash journaled; retain policies; a downstream instrument whose reads are withheld under `window:` retention either keeps its chunks or receives a `retention` truncation, never a short result | M5 | planned |
| CIW-SESS-008 | integration: replayed selection equals published; rate, pause, seek; nondeterministic from recordings | M2 | planned |
| CIW-SESS-009 | integration: `verify` detects an injected mismatch with first differing row; injected platform-record difference on a bitwise instrument yields DRIFT and exit 0; same-platform mismatch and beyond-tolerance mismatch yield exit 3; `--strict`; new session references original | M2, M3 | planned |
...
| CIW-SESS-012 | POSIX: `tests/test_deployment.py` signal tests (SIGTERM and SIGINT save; restart preserves exact JSON; resume without computation; corrupt workspace fails; save failure at shutdown propagates) and `scripts/check_container.py` (selection and result identities after SIGTERM and restart without an explicit save); Windows: `scripts/check_local.ps1` (Stop saves and verifies before terminating the owned process; Start restores under a new session identity); the signal test is skipped on Windows | M0 | satisfied (POSIX signal path) / satisfied (Windows controller path) |
| CIW-SESS-013 | integration: `tests/test_deployment.py` health over a live session creates no result; bounded failure on a silent server; non-WebSocket URL rejected; invalid snapshots (error type, version, session, selection, bounds, results) rejected | M0 | satisfied |
| CIW-SESS-014 | integration: an attached bundle is listed by `result.list` and returned by `result.get` with unchanged identities and no re-evaluation; reopening the workspace restores it; attached bundle with foreign `evidence_id` restored on reopen through its `bundle_digest`; `workspace_version: 2` refused by a v1 reader; attach copies the bundle beside `workspace.json`, a moved output directory reopens, attaching the same bundle twice is refused, `null` run-bound summary fields for bundle rows | M1 | planned |
| CIW-OPS-001 | integration: `tests/test_integration.py` (`analyze stats`, `inspect`, `send session.get`), `tests/test_deployment.py` (`serve` with `--resume` and signals; `health` exit 0 and 2, read-only, and its 3 s bound; `--bind` accepts only `127.0.0.1` and `0.0.0.0`), `scripts/check_installed.py` (`demo`, `analyze spectrum`, `inspect`), `tests/test_plsr.py` (`plsr`); `inspect` executes nothing; to be added: `send` exit 2 on an `error` envelope, `watch` event lines from a second client's update and exit 0 on normal close | M0 | partial (`watch` and the `send` error exit untested) |
...
| CIW-EXT-003 | integration: json, csv, arrow, and npz exporters registered as plugins under `ciw.exports` without service change, each writing the CIW-SESS-010 sidecar (M2); gltf and png exporters as plugins (M4) | M2, M4 (gltf, png) | planned |
...
| CIW-EXT-007 | inspection: every contract in Sections 6-12 other than CIW-INST-004 and the `inprocess` binding row depends on no language runtime, toolkit, or renderer (byte formats permitted); extensions declare majors; 6.2 table carries the majors column | M0 | satisfied |
...
| CIW-EXT-010 | inspection: each catalogue entry carries the required items; integration: entry point, saved evidence, and replay exercised together (`tests/test_replay.py` and `scripts/check_installed.py` for the oscillator; `tests/test_plsr.py` and `scripts/check_plsr_installed.py` for PLSR) | M0 | satisfied |
...
| CIW-PERF-013 | integration: `scripts/check_installed.py` runs the CLI from the built wheel; CI builds the `ciw-python-wheel` artifact, runs the container lifecycle on Linux and the native controller lifecycle (`scripts/check_local.ps1`) on Windows | M0 | satisfied |
...
| R15 | The PLSR upstream kernel API is marked `changing`; the adapter is bound to one source pin. | A pin update can change status semantics or the artifact schema without notice. | Source-manifest verification rejects any other source (CIW-EXT-009); a pin update requires a compatibility review, renewed integration checks, and a decision record (ADR-0004). |
| R16 | Autosave covers graceful shutdown only; forced termination or power loss loses changes since the last completed save. | The latest selection and results of an investigation can be lost. | `workspace.save` as the explicit checkpoint; the native controller saves and verifies before stopping; the container's 30 s grace period; the M2 journal (CIW-SESS-006) makes every mutation durable. |
...
- **Workbench-ready**: the instrument attaches through the contract with an adapter only; both frontends identify the same records; reopening is distinguished from recomputing; its conformance rows pass; the reusability report shows adapter changes above service changes. An instrument is recorded as integrated in [`docs/INSTRUMENTS.md`](INSTRUMENTS.md) only after its workbench entry point, saved evidence, and replay path have been exercised together on a representative input (CIW-EXT-010); upstream tests or a published schema alone establish standalone readiness, not workbench integration. The PLSR adapter is integrated in this sense for the headless path and is not workbench-ready until CIW-SESS-014 and CIW-VIEW-001 hold for it.
...
|---|---|---|
| **M0 — Prototype v0.1** (done) | Synthetic oscillator instrument; headless statistics and periodogram with immutable results and identities; session service over loopback WebSocket; `ciw` CLI; Godot viewport with phase portrait, energy surface, shared cursor, numerical cards; workspace save and reopen; deployment paths (5.8): native controller (Setup, Start, Status, Stop; owned hidden process; `.ciw/` data directory) and container backend (Compose service, named volume, `--bind 0.0.0.0 --resume`, healthcheck), saved shutdown on SIGINT and SIGTERM, `serve --resume`, `ciw health`; PLSR headless adapter (13.8): `ciw plsr import\|evaluate\|inspect\|replay` over a source-pinned engine writing self-contained run bundles; CI: Python matrix, Godot bridge check, wheel build with the installed-package check, container lifecycle on Linux, native lifecycle on Windows, PLSR adapter tests and installed check | Two clients change the shared selection; the same retained sample is inspected in both; the interval survives cursor moves; the viewport reconnects without stopping the service; a workspace reopens without recomputing; a service stopped by SIGTERM and restarted with `--resume` restores the selection and results without an explicit save; a pinned PLSR model and explicit sample evaluate to a stored bundle that `inspect` validates without recomputation and `replay` re-evaluates under new identities with a digest comparison; a numerical refusal is stored with exit 0 and `not_verified`. |
| **M1 — Existing-instrument adapter (reference instrument a)** | Already delivered at M0 by the PLSR headless adapter (13.8): an external engine called under a source pin with its status semantics preserved, runtime identity in provenance, separate identities, inspect versus replay, catalogue entry. Remaining: manifest and schema; subprocess binding in batch mode with the lifecycle, message families, ordering, error record, and binary frames with descriptor on the bulk plane; SDK shim; capability-driven representations; session attachment of run bundles (CIW-SESS-014); envelope v1.1 with `result.created`; unit grammar; Textual panels at tiers 0–2: numerical, temporal, spectral (CIW-VIEW-013), and the orthographic spatial preview (CIW-VIEW-016); command grammar, `help --json`, keybindings, layouts, scripts, `assert`; reductions; `ciw-proto-check` (attach, run, detach) | The oscillator (reference instrument a) attaches behind a manifest and adapter with its calculation and tolerances preserved; a PLSR bundle attached to a session is listed and reopened with its identities intact; over SSH the cursor, interval, and units move every pane of a four-pane layout in step; the reusability report shows adapter changes above service changes. |
...
Each milestone closes only when its conformance rows pass and, from M1, when `CIW-EXT-006` is met. A PERF row closes on a profile run; if no profile runner exists at the milestone, the row is recorded as `inspection (recorded, not gated)` in the milestone report and the milestone may close with that entry.

## 19. Decisions

| # | Decision | Rationale |
|---|---|---|
| D1 | Topology: an authoritative session service with the terminal and viewport as independent clients; the terminal works alone; closing the viewport never stops the service. | The v1 baseline and the Jupyter positioning; a client crash never delays sequencing; a stalled client can hold the sequencer for up to about 22 s in v1 (5.3) and removed from the sequencing path at M1 (CIW-SYNC-021). |
...
| D3 | Bulk encoding: JSON only on the client link in v1; from M1 raw NumPy buffers with an explicit descriptor on the instrument bulk plane, from M4 on the client link; Arrow IPC for tables and export. | The viewport has no Arrow reader; Arrow keeps its value where schema evolution and interchange matter. |
...
| D6 | Spectra are operations: numerical code supplied by instrument modules, sequenced, wrapped in the envelope, and persisted by the service's Operation runner as immutable results; never computed in a view. | Clients own no calculation; results carry identities; v1 already works this way. |
...
| D8 | Intervals: half-open `[start, end)`; float seconds on the v1 fields (as built); from M5 per-base `int64` ns fields are authoritative, float fields are converted once by CIW-DATA-005 and both never appear in one update; the encoding of the ns fields is owned by `docs/PROTOCOL.md`. This is a protocol minor, not a major: in a session with one time base, `cursor_s` and `interval_s` keep their v1 meaning and are accepted and reported unchanged; `cursors` and `intervals` are optional additions that are authoritative whenever present and are the only accepted form for a base that fails the precision bound. A v1 client that never sends the ns fields observes no change. | Matches code and tests; nanosecond bases matter only with several clocks. |
...
| D31 | Shutdown: the service drains, finishes in-flight operations, and saves on SIGINT or SIGTERM (POSIX); where process termination is immediate and delivers no signal (the native controller on Windows) the controller requests `workspace.save`, verifies the response, then terminates the owned process; no shutdown request in the client protocol; the deployment controller owns the process. | A protocol shutdown would let any local client stop a shared session; ownership metadata (`service.json`) and Compose already control the process; Windows termination delivers no signal, so the native controller saves by request before terminating. |
| D32 | External instruments: adapters call the upstream engine under a verified source pin and never vendor it; the first, PLSR, is a headless terminal adapter that changes no protocol (ADR-0004). | One numerical implementation to validate; one external instrument does not yet establish which further abstractions the session protocol should share. |
...
- [`docs/PROTOCOL.md`](PROTOCOL.md) — wire-level protocol for version 1: exact encodings, schemas, handshake; must satisfy every wire-visible requirement of this document (Section 2.3).
- [`docs/quickstart.md`](quickstart.md) — running the prototype from source.
- [`docs/DEVELOPMENT.md`](DEVELOPMENT.md) — development guide.
- [`docs/INSTRUMENTS.md`](INSTRUMENTS.md) — catalogue of integrated instruments: entry point, commands, input and output conventions, evidence and replay, verification state, limits.
