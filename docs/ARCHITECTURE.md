...
| **Snapshot** | The `session.snapshot` event and `session.get` response (session id, `RUN_METADATA`, selection, result summaries; from M1 also `view_settings` and `protocol_minor`); in the extension also the immutable `(revision, W, t_tick, seen)` one render tick draws from (CIW-SYNC-010). |
...
Component names across the document set differ only in wording: the **Session Service** here is the *Workbench runtime* of `README.md`, and the **Terminal Client** here is the *terminal frontend* of `README.md` and ADR-0003; the **Viewport** is the 2D/3D viewport in all three.
...
| Terminal client (`send`, `watch`, `analyze`, panels) | Zero or more | Service restart (v1: `send` exits 2 when its request goes unanswered; `watch` exits 0 on a normal close, including saved shutdown, and 2 on an abnormal close; either is re-run to reattach; panels reconnect and request a snapshot from M1) |
...
The service is a separate process from both clients so that a client crash cannot delay sequencing, a stalled client is bounded (v1: by the stall behaviour in 5.3; from M1: removed from the sequencing path, CIW-SYNC-021), and the same service serves both clients symmetrically. `ciw analyze` embeds the Session Service in the terminal process without a socket: its record and result are produced by the service code, not by a client. In-process instruments share the service's failure domain and are restricted to bounded calls; the subprocess binding is the default for anything else.
...
| Controller ↔ service | Native: `session.get` and `workspace.save` over the client link (`ciw send`), ownership in `service.json`, stop by terminating the owned process after a verified save; container: Docker healthcheck through `ciw health`, SIGTERM with a 30 s grace period | Unchanged; no shutdown request is added to the client protocol (CIW-SESS-012) |
...
| Service stops (SIGINT/SIGTERM on POSIX; native Windows: controller Stop) | Viewport shows STALE, disables shared interaction, offers Reconnect; `ciw send` exits 2; `ciw watch` exits 0 (normal close); an operation in flight completes and its result is saved before exit (CIW-SESS-012) | `ciw serve --resume` (or `--workspace`) restores the workspace saved at shutdown; `ciw health` confirms the session before clients reconnect |
...
`CIW-INST-023` (extension, M1) After a result is registered (result file written, ledger updated under the session lock) the service MUST broadcast `result.created` with `request_id: null` and payload RESULT_SUMMARY (including `source_kind`) to every connected client through the per-client queue of CIW-SYNC-021, in registration order relative to that client's selection updates; the requesting client MUST receive its response before its copy of the broadcast; an attached bundle (CIW-SESS-014) produces the same broadcast. A client MUST treat a missed or out-of-order broadcast as a reason to refresh `result.list` or request a snapshot, never as data loss; the result file exists before the broadcast is queued.

Each requirement's own marker names the milestone of the extension it belongs to; the table groups the extensions. Minors within these majors are declared per extension in `docs/PROTOCOL.md` (CIW-INST-003); a row that required a new major would say so here.

| Extension | Adds | Milestone | Majors (protocol / envelope / manifest) |
|---|---|---|---|
| Manifest and adapter | Manifest, channel descriptors, capability-driven representations, subprocess binding in batch mode and SDK shim, existing-instrument adapter, operations as instrument code, one channel namespace, determinism harness, proto-check, session attachment of run bundles | M1 | 1 / 1 / 1 |
| Unit grammar and reductions | UCUM grammar and dimension vector, display-unit preferences, min/max reductions | M1 | 1 / 1 / 1 |
| Result events and envelope v1.1 | `result.created` broadcast; protocol minor; uncertainty declarations, payload types, `reduced_of`, `checks[]`, `refused` status, stale marker; non-blocking broadcast queues | M1 | 1 / 1 / 1 |
| Terminal panels | Textual panels, tiers, command grammar, keybindings, layouts, scripts, `assert`, render tick; `view.update` and `view.changed` | M1 | 1 / 1 / 1 |
| Journal and session store | Sequence-numbered journal, session directory, replay, `verify` for operations, csv/arrow/npz exports | M2 | 1 / 1 / 1 |
| Persistence adapter | Resolver mapping the identities to an external infrastructure | M2 | 1 / 1 / 1 |
| Uncertainty and revisions | Uncertainty forms and rendering, superseding results, `t_avail`, `as_of`, live parameters, `xy` plots, `verify` for instruments | M3 | 1 / 1 / 1 |
| Binary frames and viewport resources | Bulk buffers with descriptor on the client link, coordinate-frame tree and gaps, resources by id, shared camera, rasterizer and graphics tier, gltf/png exports | M4 | 1 / 1 / 1 |
| Streaming | Credits, cancellation, heartbeats, remote and observed bindings, time bases, follow-live, recording, `host` channels | M5 | 1 / 1 / 1 |
| Spectral expansion | Welch, spectrogram, frequency cursor, link groups | M5 | 1 / 1 / 1 |
| Deployment | Observed service class profile (R11), packaging, measured workloads, fault injection | M6 | 1 / 1 / 1 |
...
A source instrument declares the kind of what it emits: recorded or synthetic evidence that stands in for a measurement is `observation` (the v0.1 oscillator's `q`, `v`, and `energy`, whose provenance says so, carry kind `observation` once descriptors exist at M1); the output of an estimator, reconstruction, or integrator run inside the session is `estimate`.
...
`CIW-DATA-016` (extension, M1 instrument bulk plane; M4 client link) A bulk frame MUST carry a descriptor sufficient to interpret its buffers without the envelope: `frame_seq`, `execution_id`, `stream` (`<instrument short id>/<output name>`), `result_id` or `channel_ids[]`, a time column or `(t0, dt, n)` with its `time_base`, and per buffer `channel_id` (`null` for the time column), `column` (`value` or a companion column of CIW-DATA-010, which never appears in `channel_ids[]`), `dtype`, `shape`, `byte_order` (`little`), `order` (`C`), `byte_offset`, `byte_length`, `unit`, and `coordinate_frame` for a spatial channel buffer. The default transfer and at-rest format is NumPy-compatible raw contiguous buffers with this descriptor; Arrow IPC (pyarrow) is the format for table payloads and export and MAY be negotiated per link. Buffers MUST NOT be embedded in text records. On the client link bulk frames share the connection with control messages, so a control broadcast waits behind the bulk in flight: a frame or resource message MUST be at most 1 MiB (larger resources are chunked), the bulk ahead of any control message is bounded by the subscription's outstanding credit (default 8 frames / 8 MiB, sized for loopback), and a client MUST grant only credit that its link drains within the 2 s of CIW-SYNC-021 (over an SSH tunnel, what its measured link rate drains in 1 s).

Instrument-link `data/frame` record (CIW-INST-008) from the state-estimation reference instrument; at M4 the service forwards the same `payload` to subscribed clients as a `frame` broadcast (a protocol minor; `bulk` replaced by the binary WebSocket message that follows), framing per `docs/PROTOCOL.md`:
...
`CIW-SESS-014` (extension, M1) A run bundle MAY be attached to a session by `result.attach {bundle_file}` (CIW-INST-022; wire form in `docs/PROTOCOL.md`). The service MUST copy the bundle unchanged into the output directory under its own basename (`run-<result uuid>.json`, 13.8; M2: `bundles/`), refusing with `storage_error` when a file of that name holds different content, MUST verify `bundle_digest` on the copy, and MUST register it as an immutable result of the workspace with its five identities and `verification_status` unchanged, `bundle_digest` as the source binding, and `bundle_file` recorded relative to the output directory exactly as `recording_file`; it MUST NOT re-evaluate the bundle on attach or on reopen, and MUST refuse with `invalid_payload`, leaving the session unchanged, a bundle whose `result_id` or `execution_id` is already in the ledger. An attached bundle MUST appear in `result.list` and `result.get` as a result whose `payload` is a `reference` to the bundle (CIW-DATA-015) with the instrument's status record as the inline summary, with views offered only per CIW-VIEW-001. `results[]` MAY therefore hold results whose `evidence_id` differs from `run.evidence_id`; RESULT_SUMMARY MUST carry `source_kind ∈ {run, bundle}`, with `channel`, `interval_s`, and `selection_revision` `null` for a bundle row, which a client MUST tolerate only when `source_kind` is present (the fields keep their v1 meaning wherever non-null: protocol minor 1.1 under CIW-INST-003). A workspace holding an attached bundle MUST be saved with `workspace_version: 2`, which a v1 reader refuses under CIW-SESS-004 naming the version; a v2 reader MUST accept `workspace_version: 1`. Until then a bundle is inspected and replayed only through the headless path.
...
`CIW-SYNC-022` (extension, M1) Channel references are bare `channel_id`s in one session namespace (the dot in `att.quat` is part of the id, never a separator). Every `channel_id` MUST be unique across the session's run and the declared outputs of every attached instrument; `attach` MUST be refused with `channel_conflict` otherwise. The selection's `channel`, `view_settings.channels`, and every channel argument in Section 12.2 use this form; in v1 the namespace is the run's channels. Channels produced by an execution belong to its result (`channels[]`, CIW-DATA-013), become addressable when the envelope is committed, and are addressed in qualified form `<channel_id>:<execution_id>` (`:` never appears inside a `channel_id`). A bare id declared only by the run resolves to the run channel; a bare id produced by results MUST be resolved by the service, when the `selection.update`, `view.update`, or command is accepted, to the most recently committed non-superseded result declaring it (journal order from M2; `created_at` at M1), and the selection, `view_settings.channels`, and every broadcast MUST carry the qualified form so that a later execution never changes what an existing revision denotes; `compare` pairs, residuals, and `assert` expressions (`att.quat:execution-9d0e…@cursor`) use it too.
...
`CIW-SYNC-009` (extension; † M1, camera M4, follow and freq_cursor M5) `view_settings` (empty in v1) MUST hold exactly the fields below when populated; a change to any of them except `focus` MUST be a revisioned update that never alters stored data, sharing one revision counter with the selection: sent as `view.update {expected_revision, <fields>}`, applied by the selection sequencer under the same lock with the conflict rule of CIW-SYNC-003, and broadcast as `view.changed {selection, view_settings, revision}` (without `focus`). One sequenced mutation MAY change selection and view-settings fields together (CIW-SYNC-011); it is one revision and one `view.changed` with no separate `selection.changed`, so the discard rule of CIW-SYNC-004 applies unchanged. From M1 `session.snapshot` and the `session.get` response MUST include `view_settings` alongside `protocol_minor` (CIW-INST-003). `focus` is per-client presentation state: never broadcast, persisted only as the saving client's value, and MUST NOT change in another client.
...
`CIW-VIEW-006` (extension, M1 tiers 0–2; M4 graphics) The terminal client MUST classify the terminal at startup and on resize and select the highest confirmed tier, overridable by `--tier`: 0 cell; 1 braille 2×4 and half-block 1×2 with 256 colours; 2 true colour; 3 a graphics protocol in the order `kitty > iterm2 > sixel`. Tier 3 MUST be confirmed by terminal response (Kitty via the APC graphics query, Sixel via DA1 attribute 4), never by `TERM` alone; each query waits max(100 ms, 4 × the measured DA1 round trip), capped at 1 s, and a timeout leaves the tier unconfirmed (`tier 2 (unconfirmed 3)` in the status line); iTerm2, having no graphics query, is the one environment-variable exception (`TERM_PROGRAM=iTerm.app` or `LC_TERMINAL=iTerm2`), with a failed first image write falling back one tier; under tmux or screen tier 3 requires passthrough and defaults to 2. A backend error at runtime MUST fall back one tier. Tier-3 payloads MUST be at most 256 KiB per pane per frame; a pane whose tier-3 write misses one tick period on two consecutive ticks (Kitty additionally on a missing `q=0` acknowledgement within one tick) drops to tier 2 for the rest of the session.
...
| Client transport | Loopback WebSocket `ws://127.0.0.1:8765`, text JSON (v1); binary frames on the same connection (M4); Unix socket and TCP with token or TLS (M5) | One transport both clients speak; forwardable over SSH; open to a browser client at no protocol cost; head-of-line delay bounded by the client's credit (CIW-DATA-016). |
...
| benchmark | two runs: (a) CI regression against a recorded baseline on the CI machine class, failing on a regression above 20 % and never asserting the budget; (b) budget assertion on a named profile runner (self-hosted, or a documented machine whose report is attached to the milestone record), which a PERF row must pass to close a milestone |
...
| CIW-INST-007 | integration: `ciw-proto-check` drives every state with timeouts enforced and transitions journaled; withholding `finalized` past 30 s yields Failed, `status: failed`, `truncated_at.reason = instrument_failed` (M1); the same case first sends `cancel` (M5) | M1 (batch), M5 (stream, cancel) | planned |
...
| CIW-INST-018 | integration: same suite over inprocess, subprocess, `ssh://localhost`; shim wraps an in-process instrument; an in-process instrument that overruns 10× its budget is marked Failed, and the abandoned-thread cap and `resource_exhausted` refusal hold without delaying the sequencer | M1 (subprocess), M5 (remote) | planned |
...
| CIW-SYNC-003 | unit: stale revision rejected without mutation; boolean and negative revisions rejected; missing `expected_revision` rejected; to be added: an update naming only `expected_revision` rejected with `invalid_payload`; an update restating the current values increments `revision` | M0 | satisfied (stale, boolean, and missing-field rejection tested) / partial (rejection of an update naming only `expected_revision` and the revision increment on a restated update untested) |
...
| CIW-SYNC-011 | unit: arrival leaves revision unchanged with follow off; ≤ 1 follow mutation per `follow_period_ms` with follow on, origin `system`; an explicit cursor or window update sets `follow: off` in one revision and one `view.changed`; integration: two following clients send no updates and stay at one revision | M5 | planned |
...
| CIW-SYNC-022 | unit: duplicate `channel_id` refused with `channel_conflict`; bare ids resolve for run and result channels; with two executions of one instrument a bare id resolves to the latest at acceptance, the broadcast carries the qualified form, each qualified id resolves to its own execution, and an earlier revision is unchanged by a later commit | M1 | planned |
...
| CIW-VIEW-019 | integration: Godot smoke (renders from delivered records, `selection.update` with the observed revision, playback moves only the cursor); `scripts/check_godot.py` (closing the viewport leaves the service running); to be added: STALE on disconnect | M0 | partial (STALE on disconnect implemented but untested) |
...
| CIW-SESS-012 | POSIX: `tests/test_deployment.py` (SIGTERM and SIGINT save; restart preserves exact JSON; resume without computation; corrupt workspace fails; save failure at shutdown propagates) and `scripts/check_container.py` (selection and result identities after SIGTERM and restart without an explicit save); Windows: `scripts/check_local.ps1` (Stop saves and verifies before terminating the owned process; Start restores under a new session identity); the signal test is skipped on Windows | M0 | satisfied (POSIX signal path and Windows controller path) |
...
| CIW-SESS-014 | integration: an attached bundle is listed by `result.list` and returned by `result.get` with unchanged identities and no re-evaluation; copied beside `workspace.json`; restored on reopen, also from a moved output directory, through its `bundle_digest` despite a foreign `evidence_id`; `workspace_version: 2` refused by a v1 reader; attaching the same bundle twice refused; `null` run-bound summary fields for bundle rows | M1 | planned |
...
| # | Decision | Rationale |
|---|---|---|
| D1 | Topology: an authoritative session service with the terminal and viewport as independent clients; the terminal works alone; closing the viewport never stops the service. | The v1 baseline and the Jupyter positioning; a client crash never delays sequencing; a stalled client can hold the sequencer in v1 (5.3) and is removed from the sequencing path at M1 (CIW-SYNC-021). |
...
| D8 | Intervals: half-open `[start, end)`; float seconds on the v1 fields (as built); from M5 per-base `int64` ns fields are authoritative and float fields are converted once, never both in one update, as the protocol minor of CIW-DATA-005 whose encoding `docs/PROTOCOL.md` owns. | Matches code and tests; nanosecond bases matter only with several clocks. |
...
| D31 | Shutdown: saved shutdown on SIGINT or SIGTERM (POSIX); where termination delivers no signal (the native controller on Windows) the controller requests `workspace.save`, verifies the response, then terminates the owned process; no shutdown request in the client protocol; the deployment controller owns the process (CIW-SESS-012). | A protocol shutdown would let any local client stop a shared session; ownership metadata (`service.json`) and Compose already control the process. |
