...
| Calibration contract and deterministic bench | Calibration profile and its record, raw immutability, corrected channels as derived result channels, calibration state, profile refusal, deterministic fixture set, bench report and metric set, physical-bench declarations, execution placement, replay from local files | CIW-CAL-001, CIW-CAL-002, CIW-CAL-003, CIW-CAL-004, CIW-CAL-005, CIW-CAL-006, CIW-CAL-007, CIW-CAL-008, CIW-CAL-009, CIW-CAL-010, CIW-CAL-011 | M1; M3 (physical bench, CIW-CAL-009) |
...
| Uncertainty and revisions | Uncertainty forms and rendering, superseding results, `t_avail`, `as_of`, live parameters, `xy` plots, `verify` for instruments, estimator declarations and check vocabulary, ablations as compared executions | CIW-DATA-010, CIW-DATA-017, CIW-DATA-018, CIW-INST-015, CIW-INST-025, CIW-VIEW-012, CIW-VIEW-018, CIW-OPS-013, CIW-EXT-002 | M3 |
...
Manifest of reference instrument (b), a batch linear-Gaussian path-state estimator, trimmed to the declared fields (`lag_s > 0` re-solves as measurements accumulate and emits superseding results; at M3 both outputs run in `batch` mode over the recorded input, `mode: stream` and `overflow` taking effect at M5):
...
```json
{
  ...
  "outputs": [
    {"name": "state", "mode": "batch", "operation_id": "path_state.solve.v1", "representations": ["numerical", "temporal", "spatial2d"],
     "channels": [
...
| compare | `compare`, `residual`, `assert`, `bench` | `compare result-3f9c… result-a1b2…`, `residual path.lat0 ref.lat`, `bench --instrument org.ciw.ref.timeseries --corpus examples/calibration/org.ciw.ref.timeseries/` |
...
`CIW-CAL-011` (extension, M1) One real instrument, one calibration profile, one golden corpus, and one bench report MUST be **replayable entirely from local files**: the profile bundle, the corpus under `examples/calibration/<instrument>/`, and the report MUST each be readable and validated without the service and without network access, and replay MUST re-evaluate under new execution and result identities without modifying its sources (CIW-INST-021). A saved workspace MUST retain the calibration reference and digests of CIW-CAL-003 beside the manifest hash and normalization mapping of CIW-EXT-011, and reopening MUST restore them without recomputation (CIW-SESS-002), so a reopened session can name the profile its numbers came from; independent re-verification of those digests on read arrives with CIW-SESS-016 at M2, which is stated as a limit of the M1 form.
...
```json
{
  ...
  "calibration": {
    ...
    "instrument": {"id": "org.ciw.ref.timeseries", "serial_number": "TS-000417"},
    ...
    "response": {"channel_id": "ts.raw", "unit": "V", "values": ["…"]},
    ...
    "transform_uncertainty": {"gain": {"stddev": 5.2e-6, "unit": "1"}, "offset": {"stddev": 9.2e-8, "unit": "m"}},
...
Bench report (CIW-CAL-007, CIW-CAL-008), showing residuals and per-metric pass or fail:
...
```json
{
  ...
  "calibration": {"state": "calibrated", "profile_result_id": "result-5c1d…", "profile_digest": "sha256:0f37…",
                  "applied_at": "2026-09-20T09:58:12+00:00", "valid_until": "2027-08-11T00:00:00+00:00"},
  "corpus": {"path": "examples/calibration/org.ciw.ref.timeseries/", "digest": "sha256:9b04…", "runtime_identity_matches": true},
  "fixtures": [
    {"fixture": "constant-2v", "class": "zero and constant", "metrics": [
      {"metric": "bias", "value": -3.0e-7, "residual": -3.0e-7, "unit": "m", "tolerance": {"abs": 5.0e-7, "declared_in": "corpus policy"}, "pass": true},
      {"metric": "repeatability", "value": 6.1e-8, "residual": null, "unit": "m", "tolerance": {"abs": 1.0e-7, "declared_in": "corpus policy"}, "pass": true},
      {"metric": "noise floor", "value": 7.4e-8, "residual": null, "unit": "m", "tolerance": {"abs": 1.0e-7, "declared_in": "corpus policy"}, "pass": true}]},
    {"fixture": "sweep-0p1-to-20hz", "class": "sine sweep and chirp", "metrics": [
      {"metric": "linearity", "value": 1.4e-6, "residual": 1.4e-6, "unit": "m", "tolerance": {"abs": 1.0e-6, "declared_in": "corpus policy"}, "pass": false},
      {"metric": "phase error", "value": 0.0031, "residual": 0.0031, "unit": "rad", "tolerance": {"abs": 0.005, "declared_in": "corpus policy"}, "pass": true}]},
    {"fixture": "impulse-t3", "class": "step and impulse response", "metrics": [
      {"metric": "timing error", "value": 2.1e-4, "residual": 2.1e-4, "unit": "s", "tolerance": {"abs": 5.0e-4, "declared_in": "corpus policy"}, "pass": true}]}
  ],
  "unavailable_metrics": [{"metric": "drift", "reason": "fixture class 'clock drift and irregular sampling' is available at M5"}],
  "summary": {"fixtures": 3, "metrics_passed": 5, "metrics_failed": 1, "exit_code": 1}
}
```
...
| CIW-DATA-010 | unit: each form; absent never zero; symmetry; sampled Cholesky; tangent-space covariance; companion naming; undeclared column rejected; a v1 result declares no uncertainty explicitly and no consumer path reads absence as zero | M3 | planned |
...
| CIW-INST-025 | unit: `solver` and `validity_domain` required of an estimator manifest; each declared check name evaluated against its stated pass condition and per-instrument tolerance; a posterior outside the validity domain marked `refused` with category `outside_declared_domain` or as a failed check, never presented with an unqualified covariance; a check sets no `verification_id`; the joint posterior covariance and the information eigenbasis delivered in the result payload rather than as channels; inspection: the adapter forms no explicit inverse, asserted on an ill-conditioned information matrix | M3 | planned |
...
| CIW-CAL-001 | unit: a profile bundle carrying every listed field validates, and one missing the serial number, the validity interval, the traceability, the fit residuals, the transform uncertainty, or the raw run's `evidence_id` is refused; the raw calibration run carries instrument identity, serial number, environment, and reference standard in `acquisition{}`; no profile field is treated as a sixth identity | M1 | planned |
...
| CIW-CAL-003 | unit: a corrected channel is `derived`, `derived_from[]` names kinds only, provenance names the profile's `result_id`, `bundle_digest`, and validity interval, and the correction is recorded as an explicit `convert`-family derivation with its method and declared domain; a gain, offset, or frame change applied without a declared transform refused under CIW-EXT-011; a bare channel id naming a calibrated quantity refused in favour of `<channel_id>:<execution_id>` | M1 | planned |
| CIW-CAL-004 | unit: the envelope's `calibration{}` state set and fields; `calibrated` sets neither `verification_id` nor `verification_status`, a verified result without a profile stays `uncalibrated`, and a `checks[]` entry sets neither axis; integration: an uncorrected result labelled `uncalibrated` in both frontends | M1 | planned |
| CIW-CAL-005 | unit: a missing profile, one expired at the record's acquisition time, and one whose instrument identity or serial number does not match are each refused with `calibration_unavailable` naming the condition, writing no result and leaving the session unchanged, with no uncorrected fallback, no extrapolated transform, and no substituted profile; inspection: the code declared as a minor addition in `docs/PROTOCOL.md` under CIW-INST-003 | M1 | planned |
| CIW-CAL-006 | unit: every M1 fixture class present with its input, expected result, golden output, and per-field tolerance policy, a field without a declared tolerance compared bitwise; the M5 classes stored only as refusal cases, never as a reason to widen `validate_run`; integration: a corpus whose recorded runtime identity differs from the installed one fails as stale | M1, M5 (clock drift, irregular sampling, missing samples, non-finite values) | planned |
| CIW-CAL-007 | unit: the report carries per fixture and per metric the computed value, the residual, the tolerance with the source that declared it, and a pass or fail, plus the instrument, profile, corpus, and runtime identities, and keeps `verification_status: not_verified` with a null `verification_id`; a failed metric exits 1 and never 3; golden: the report rendering at tier 0; inspection: no CAL row is verified by the `benchmark` method | M1 | planned |
| CIW-CAL-008 | unit: each of the seven metrics computed over its fixture class and compared against the tolerance declared in the corpus policy of CIW-EXT-012; a metric whose fixture class is later than the milestone reported unavailable with that milestone rather than as passing; inspection: no global accuracy figure is stated | M1 | planned |
| CIW-CAL-009 | unit: a bench run without an independent `reference` channel, without the stimulus recorded beside the response, or without the controlled-or-logged marking of temperature, mounting, cable configuration, gain, sampling clock, and geometry refused; a spatial or motion bench run without alignment points or an explicit coordinate frame refused; integration: a physical bench run reopened with every declaration intact | M3 | planned |
...
The calibration increment of 13.10 closes against these seven items. Each names the identifier whose Section 16.4 row records the check — 16.4 stays the single place a check is recorded — and the milestone whose close it gates. Contract and software-bench work needs no hardware and gates M1 with the adapter seam; the physical bench and the first calibrated real instrument gate reference instrument (b) at M3.
...
| 7 | An expired, mismatched, or missing calibration profile fails clearly. | CIW-CAL-005 | M1 |

Items 3, 4, and 5 carry a later milestone because the work they depend on is scheduled there, not because the gate is soft; an item may not be recorded as closed at M1 on the strength of the part of it M1 delivers. The deliverable the gate aims at is one real instrument, one calibration profile, one golden corpus, and one bench report replayable entirely from local files (CIW-CAL-011).
