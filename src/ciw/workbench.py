...
REPRODUCED_KINDS = DECLARED_KINDS - {"proved-heat", "julia-oscillator"}
...
INSTRUMENT_ROLES = frozenset({"ppda", "tbrt", "mcur", "stfe", "gsie", "cbsr", "fdir", "oit", "sra", "scr", "cse", "rci", "fsrt", "jspt", "gte", "plsr", "ftr", "csg", "cggt", "isgt", "tsde", "energy", "exchange", "thermal", "machine", "julia"})
...
def _validate_links(record, bundles):
    ...
    for receipt in native.get("replay_receipts", []):
        ...
        if record["kind"] == "julia-oscillator":
            # Floating-point replay claims agreement under the declared tolerance;
            # byte identity is recorded in the receipt rather than required.
            _workflow("julia-oscillator").validate_replay(original["native"], native, receipt)
        ...
        if any((old["numerical_result_id"] != new["numerical_result_id"] and record["kind"] != "julia-oscillator") or
...
class Workbench:
    ...
    def inspect_experiment(self, payload):
        ...
        with self._lock:
            ...
            if record["kind"] == "julia-oscillator":
                from .julia_oscillator_view import project as project_julia
                return project_julia(record, source, declaration, self._revision)
            if record["kind"] == "acquired-calibrated-window":
