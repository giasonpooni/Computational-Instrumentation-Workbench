"""Authoritative scientific records and computations for the first CIW instrument.
...
def run_metadata(run: dict) -> dict:
    """Return a detached small description without scientific or render arrays."""
    validate_run(run)
    return {
        "run_id": run["run_id"], "evidence_id": run["evidence_id"],
        "instrument": run["instrument"], "metadata": deepcopy(run["metadata"]),
        "channels": {name: {"unit": channel["unit"]}
                     for name, channel in run["channels"].items()},
    }


def inspect_sample(run: dict, time_s: float) -> dict:
    """Resolve a cursor to the nearest source sample, taking the earlier tie."""
    validate_run(run)
    target = _number(time_s, "time_s")
    ...
    return {
        ...
        "values": {name: float(channel["values"][index])
                   for name, channel in run["channels"].items()},
