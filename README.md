...
| Language-neutral model core | `ciw.model-spec.v1` with consistent unit rescaling, typed-port composition and generated LaTeX views; a pinned Julia 1.10.12 worker under the SCR seam for Tsit5 simulation, linearization, HiGHS measurement selection and ModelingToolkit Jacobians. Terminal records only; not a session kind ([model core](docs/MODEL_CORE.md)). |
