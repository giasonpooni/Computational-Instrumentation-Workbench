...
def test_builtin_is_available_without_hardware_or_repository_bindings():
    workbench = Workbench()
    assert {row["operation_id"] for row in workbench.describe_operations() if row["available"]} == {OPERATION, "ciw.encoder-position.v1", "ciw.thermal-observer.v1", "ciw.project-graph.v1"}
...
def test_shared_session_live_transport_save_restore_and_reanalysis(retained, tmp_path):
    ...
    assert {o["operation_id"] for o in restored.workbench.describe_operations() if o["available"]} == {OPERATION, "ciw.encoder-position.v1", "ciw.thermal-observer.v1", "ciw.project-graph.v1"}
