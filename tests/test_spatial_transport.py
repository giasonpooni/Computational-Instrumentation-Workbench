    ...
    async def exercise():
        ...
        async with serve(bridge.handler, "127.0.0.1", 0) as listener:
            ...
            async with connect(url) as native, connect(url + "/spatial", origin=ORIGIN) as spatial:
                ...
                assert {item["operation_id"] for item in restored.describe_operations() if item["available"]} == {"ciw.energy-accuracy.v1", "ciw.encoder-position.v1", "ciw.thermal-observer.v1", "ciw.project-graph.v1"}
